...
- Accepted TCP flows are bridged to the local Bifrost listener with a SOCKS5 CONNECT handshake, so Bifrost can reuse its existing SOCKS, TLS, rules, and traffic-recording paths.
...
- `Sources/BifrostEnhancedProxyExtension`: `NETransparentProxyProvider` TCP flow relay.
