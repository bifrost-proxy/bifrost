...
async fn install_ca(
    ...
    if mode == InstallMode::ManagedAutoTrust {
        return install_ios_configurator_ca(cert_path.clone(), device_id);
    }
...
fn install_ios_configurator_ca(cert_path: PathBuf, device_id: String) -> Response<BoxBody> {
    ...
    tracing::info!(
        ...
        "audit: iOS CA profile install requested through Apple Configurator"
    );
    ...
    let profile_path =
        std::env::temp_dir().join(format!("bifrost-ca-{}.mobileconfig", Uuid::new_v4()));
