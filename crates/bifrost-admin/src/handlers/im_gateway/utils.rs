    ...
    match detail {
        Some(d) => {
            ...
            let external_session_id = external_session_id(
                d.external_thread_id
                    .as_deref()
                    .or(context.external_thread_id.as_deref()),
                d.external_conversation_id
                    .as_deref()
                    .or(context.external_conversation_id.as_deref()),
            );
            let overview = build_im_status_overview(
                channel,
                &ImStatusOverview {
                    work_dir,
                    runner_type,
                    ...
                    reasoning_effort_text: &reasoning_effort_text,
                    reasoning_summary_text: &reasoning_summary_text,
                    external_session_id,
                    user_turn_count: d.user_turn_count,
        ...
        None => {
            ...
            let reasoning_effort_text = bifrost_agent::format_optional_status_text(
                context.model_reasoning_effort.as_deref(),
            );
            let reasoning_summary_text = bifrost_agent::format_optional_status_text(
                context.model_reasoning_summary.as_deref(),
            );
            let overview = build_im_status_overview(
                channel,
                &ImStatusOverview {
                    ...
                    external_session_id: external_session_id(
                        ...
                        context.external_conversation_id.as_deref(),
                    ),
                    user_turn_count: 0,
                    status: if channel.status == "Ready" {
                        "Ready（新会话）"
                    } else {
                        channel.status
                    },
...
pub(super) struct ImStatusChannelContext<'a> {
    pub(super) provider: &'a ImProviderConfig,
    pub(super) device_name: &'a str,
    pub(super) session_key: &'a str,
    pub(super) queue_info: &'a str,
    pub(super) status: &'a str,
}

pub(super) fn build_active_im_status_text(
    ...
    let runner_id = status
        ...
        .unwrap_or("N/A");
    let model_text = bifrost_agent::format_model_ref(
        status.model.as_deref().or(context.model.as_deref()),
        status
            .model_provider
            .as_deref()
            .or(context.model_provider.as_deref()),
    );
    let reasoning_effort_text = bifrost_agent::format_optional_status_text(
        status
            .model_reasoning_effort
            .as_deref()
            .or(context.model_reasoning_effort.as_deref()),
    );
    let reasoning_summary_text = bifrost_agent::format_optional_status_text(
        status
            .model_reasoning_summary
            .as_deref()
            .or(context.model_reasoning_summary.as_deref()),
    );
    let external_session_id = external_session_id(
        status
            .external_thread_id
...
struct ImStatusOverview<'a> {
    work_dir: &'a str,
    runner_type: &'a str,
    runner_id: &'a str,
    model_text: &'a str,
    reasoning_effort_text: &'a str,
    reasoning_summary_text: &'a str,
    external_session_id: &'a str,
    user_turn_count: usize,
    status: &'a str,
}

fn build_im_status_overview(
    channel: &ImStatusChannelContext<'_>,
...
fn external_session_id<'a>(
    external_thread_id: Option<&'a str>,
    external_conversation_id: Option<&'a str>,
) -> &'a str {
    external_thread_id
        .map(str::trim)
...
pub(super) fn resolve_im_status_runtime_context(
    ...
    let configured_runner_id = runner_id_override.or_else(|| {
        agent_config
            .runner
            .as_ref()
            .and_then(|runner| runner.custom_runner_id())
    ...
    bifrost_agent::StatusRuntimeContext {
        ...
        model_provider: model_config.model_provider.or(model_config.model_source),
        model_reasoning_effort: model_config.reasoning_effort,
        model_reasoning_summary: model_config.reasoning_summary,
        external_conversation_id: state
            .as_ref()
