    ...
    if let Some(session_key) = rest.strip_prefix("/sessions/") {
        ...
        if req.method() == Method::GET {
            ...
            if let Some(detail) = detail {
                return json_response(&session_detail_with_active_status(detail, active_status));
            }
            if let Some(detail) = external_runner_session_detail(&session_key).await {
                return json_response(&session_detail_with_active_status(detail, active_status));
            }
            if let Some(status) = active_status {
                return json_response(&active_status_session_detail(status));
            }
...
fn active_status_session_detail(status: bifrost_agent::ActiveTurnStatus) -> serde_json::Value {
