...
async fn spawn_runner_command_feishu_server() -> (
    String,
    ...
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind runner command Feishu fixture");
    let address = listener.local_addr().expect("Feishu fixture address");
            ...
            tokio::spawn(async move {
                let service = service_fn(move |request: hyper::Request<hyper::body::Incoming>| {
                    ...
                    async move {
                        ...
                        } else if path.ends_with("/application/v7/applications/cli_test/ability") {
                            r#"{"code":0,"msg":"success"}"#
                        } else if path.ends_with("/im/v1/messages/om_runner/reply") {
                            r#"{"code":0,"msg":"success","data":{"message_id":"om_reply"}}"#
                        } else {
...
#[tokio::test]
pub(super) async fn runner_command_switches_direct_provider_and_resyncs_feishu_menu() {
    let (base_url, server, mut requests) = spawn_runner_command_feishu_server().await;
    let temp_dir = tempfile::tempdir().expect("temp data dir");
    let _env_guard = EnvGuard::set_data_dir(temp_dir.path());
    let service = ImGatewayService::new(temp_dir.path());
    let mut provider = test_provider();
    provider.id = "runner-menu-provider".to_string();
    ...
    provider.agent_config = Some(ImProviderAgentConfig {
        runner: Some(bifrost_agent::AgentRunnerMode::Custom("codex".to_string())),
        work_dir: Some(temp_dir.path().display().to_string()),
        base_instructions: None,
        developer_instructions: None,
        user_instructions: None,
    });
    ...
    let mut external = service.external_cli_config_store.load();
    external.runners.insert(
        "claude-code".to_string(),
        crate::im_gateway::external_cli::ExternalCliAgentSettings {
            enabled: true,
    ...
    let handled = handle_im_runner_command(
        "/runner claude-code",
        "runner-menu-provider:ou_owner",
        ImRunnerCommandContext {
            ...
            session_manager: &service.agent_session_manager,
    ...
    assert_eq!(
        service
            .provider_store
            .get(&provider.id)
            .and_then(|provider| provider.agent_config)
            .and_then(|config| config.runner)
            .and_then(|runner| runner.custom_runner_id().map(str::to_string)),
    ...
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while paths.len() < 2 {
            if let Some(path) = requests.recv().await {
                paths.push(path);
            }
        }
    })
    .await
    .expect("reply and menu requests timed out");
    assert!(paths
        .iter()
        .any(|path| path.ends_with("/im/v1/messages/om_runner/reply")));
    server.abort();
}
