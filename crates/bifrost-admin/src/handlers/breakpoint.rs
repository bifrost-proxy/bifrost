...
async fn resume(
    ...
    if let Err(error) = state.breakpoint_manager.resume(&request_id, &phase, edit) {
        return match error {
            BreakpointResumeError::PhaseMismatch => {
                error_response(StatusCode::CONFLICT, "phase mismatch")
            }
            BreakpointResumeError::NotFound => error_response(StatusCode::NOT_FOUND, "not found"),
        };
    }
    if let Some(pm) = push_manager {
