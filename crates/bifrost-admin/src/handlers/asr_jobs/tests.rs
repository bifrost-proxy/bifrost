...
mod tests {
    ...
    fn running_task_allows_concurrency_update_but_rejects_runtime_risk() {
        ...
        let mut completed_for_migration = completed.clone();
        completed_for_migration.status = FileStatus::Success;
        let failed_key = "legacy-failed".to_string();
        let mut failed_for_migration = completed.clone();
        failed_for_migration.status = FileStatus::Failed;
        failed_for_migration.error = Some("legacy diarization failure".to_string());
        save_file_store(
            &task.id,
            &FileStore {
                version: TASK_STORE_VERSION,
                files: BTreeMap::from([
                    (completed_key.clone(), completed_for_migration),
                    (failed_key.clone(), failed_for_migration),
                ]),
            },
        )
        .unwrap();
        let preserve_migration_update = UpdateTaskRequest {
            transcription_mode: Some(AsrTranscriptionMode::Standard),
            transcription_prompt: None,
            requeue_existing_files: Some(false),
            name: None,
            audio_dir: None,
            recursive: None,
            enabled: None,
        ...
        let migrated = update_task_config(&task.id, preserve_migration_update).unwrap();
        assert_eq!(migrated.transcription_mode, AsrTranscriptionMode::Standard);
        let preserved = load_file_store(&task.id);
        let preserved_record = preserved.files.get(&completed_key).unwrap();
        assert_eq!(preserved_record.status, FileStatus::Success);
        assert!(preserved_record.output_text_path.is_some());

        let preserve_moss_update = UpdateTaskRequest {
            transcription_mode: Some(AsrTranscriptionMode::MossJoint),
            transcription_prompt: None,
            requeue_existing_files: Some(false),
            name: None,
        ...
        let migrated = update_task_config(&task.id, preserve_moss_update).unwrap();
        assert_eq!(migrated.transcription_mode, AsrTranscriptionMode::MossJoint);
        let preserved = load_file_store(&task.id);
        assert_eq!(
            preserved.files.get(&completed_key).unwrap().status,
            FileStatus::Success
        );
        let failed_record = preserved.files.get(&failed_key).unwrap();
        assert_eq!(failed_record.status, FileStatus::Failed);
        assert!(failed_record
            .error
            .as_deref()
            .unwrap()
            .starts_with(&format!("moss_non_retryable_v{}:", env!("CARGO_PKG_VERSION"))));
        assert!(failed_record
            .error
            .as_deref()
            .unwrap()
            .contains("legacy diarization failure"));
    }
