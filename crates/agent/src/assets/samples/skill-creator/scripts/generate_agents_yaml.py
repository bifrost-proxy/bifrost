#!/usr/bin/env python3
"""
Agents YAML Generator - Creates agents/openai.yaml for a skill folder.

Usage:
    generate_agents_yaml.py <skill_dir> [--name <skill_name>] [--interface key=value]
"""
...
def read_frontmatter_name(skill_dir):
    ...
    try:
        ...
        # Fallback: parse name field with regex
        name_match = re.search(r"^name:\s*(.+)$", frontmatter_text, re.MULTILINE)
        if name_match:
            return name_match.group(1).strip()
        print("[ERROR] Could not parse frontmatter 'name' field.")
        return None
...
def write_agents_yaml(skill_dir, skill_name, raw_overrides):
...
def main():
    ...
    result = write_agents_yaml(skill_dir, skill_name, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)
