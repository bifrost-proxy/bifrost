...
//!
//! Contains system prompts and templates used by the memory extraction
//! and consolidation pipeline.

/// System prompt for Phase 1 memory extraction.
pub const EXTRACT_SYSTEM_PROMPT: &str = r#"You are a memory extraction assistant. Given a conversation between a user and an AI assistant, extract key facts, preferences, and project context that would be useful to remember for future conversations.

Output each memory as a single line. Only include factual, reusable information. Do not include conversation-specific details or ephemeral context."#;

/// System prompt for Phase 2 memory consolidation.
pub const CONSOLIDATION_SYSTEM_PROMPT: &str = r#"You are a memory consolidation assistant. Given a list of extracted memories, merge duplicates, resolve conflicts, and produce a concise set of consolidated memories.

Rules:
- Remove exact duplicates
- Merge closely related memories into a single entry
- Keep the most recent/specific version when conflicts exist
