...
#[tokio::test]
async fn response_stream_worker_exposes_stream_event_and_initial_output() {
    let temp_dir = TempDir::new().unwrap();
    let engine = ScriptEngine::new(ScriptEngineConfig {
        scripts_dir: temp_dir.path().to_path_buf(),
        ..Default::default()
    ...
    let script = r#"
        stream.mode = "transform";
        stream.output = { event: "ready", data: "started" };
        stream.onEvent = () => ({ data: stream.event.data });
    "#;
    ...
    assert!(matches!(
        &worker.take_initial_step().outputs[0],
        StreamScriptOutput::Event { event: Some(event), data, .. }
            if event == "ready" && data == "started"
    ));
    let step = worker
        .event(StreamScriptEvent {
    ...
    assert!(matches!(
        &step.outputs[0],
        StreamScriptOutput::Event { data, .. } if data == "current"
    ));
}

#[tokio::test]
async fn response_stream_worker_rejects_unproductive_mock() {
    let temp_dir = TempDir::new().unwrap();
    let engine = ScriptEngine::new(ScriptEngineConfig {
        scripts_dir: temp_dir.path().to_path_buf(),
        ..Default::default()
    ...
    let error = engine
        .create_response_stream_worker(
            "inline-stream",
            Some("stream.mode = 'mock';"),
            &stream_test_response(),
            &stream_test_context(),
        )
        .await
        .err()
        .expect("mock script without next must fail");
    assert!(error.to_string().contains("requires stream.next"));
}

#[tokio::test]
async fn response_stream_worker_rejects_excessive_callback_outputs() {
    let temp_dir = TempDir::new().unwrap();
    let engine = ScriptEngine::new(ScriptEngineConfig {
        scripts_dir: temp_dir.path().to_path_buf(),
        ..Default::default()
    ...
    let worker = engine
        .create_response_stream_worker(
            "inline-stream",
            Some("stream.mode = 'transform'; stream.onEvent = () => Array(1025).fill('x');"),
            &stream_test_response(),
            &stream_test_context(),
        )
        .await
        .unwrap();
    let error = worker
        .event(StreamScriptEvent::default())
        .await
        .expect_err("oversized output array must fail");
    assert!(error.to_string().contains("output count exceeds"));
}
