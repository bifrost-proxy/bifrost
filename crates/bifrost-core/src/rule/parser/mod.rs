...
fn split_rule_parts(line: &str) -> Vec<String> {
    ...
    while let Some(c) = chars.next() {
        match c {
            ...
            // A leading `//` is the protocol-relative scheme prefix ("match all
            // protocols", e.g. `//*.example.com`), NOT an (empty) regex. Only treat
            // a leading `/` as the start of a `/regex/` token when it is not `//`.
            '/' if !in_regex
                && !in_backtick
                && current.is_empty()
                && brace_depth == 0
...
reqHeaders://{test=1}"#;
    ...
    #[test]
    fn test_parse_whistle_style_wildcard_exclude_filter() {
        let rules = parse_line("example.com host://127.0.0.1 excludeFilter://*/alice/*").unwrap();
        assert_eq!(rules.len(), 1);
        assert!(rules[0].include_filters.is_empty());
        assert_eq!(rules[0].exclude_filters.len(), 1);
    }
...
"#;
    ...
    fn test_protocol_relative_scheme_prefix_not_split_as_regex() {
        ...
        assert_eq!(parts2[0], "//example.com");

        // a genuine /regex/ token is still tokenized as regex
