...
fn test_cached_candidates_recheck_header_filters_per_request() {
    ...
    let mut rules = parser
        .parse_line(
            "multi-demand.local host://127.0.0.1:3000 includeFilter://h:x-tt-env-fe=featA-web",
        )
        .unwrap();
    rules.extend(
            parser
                .parse_line(
                    "multi-demand.local host://127.0.0.1:3001 includeFilter://h:x-tt-env-fe=featA-mobile",
                )
                .unwrap(),
        );
    let resolver = RulesResolver::new(rules);
    ...
    let ctx_web = RequestContext::from_url("http://multi-demand.local/api")
        .with_header("x-tt-env-fe", "featA-web");
    ...
    let ctx_mobile = RequestContext::from_url("http://multi-demand.local/api")
        .with_header("x-tt-env-fe", "featA-mobile");
...
fn test_cached_candidates_recheck_request_modify_filters_per_request() {
    let rules = vec![
        create_test_rule_with_filters(
            ...
            Protocol::ReqHeaders,
            "X-Selected-Env=web",
            vec![parse_filter("h:x-tt-env-fe=featA-web").unwrap()],
            vec![],
        ),
        create_test_rule_with_filters(
            "multi-modify.local",
            ...
            vec![parse_filter("h:x-tt-env-fe=featA-mobile").unwrap()],
            vec![],
        ),
    ];
    let resolver = RulesResolver::new(rules);
