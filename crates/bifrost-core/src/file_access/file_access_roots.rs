...
pub enum RootEditError {
    ...
    Write(String),
    #[error("path is not valid UTF-8 and cannot be stored in TOML: {0}")]
    NonUtf8Path(PathBuf),
}
...
/// Convert a `Path` to a TOML-storable UTF-8 `String`, rejecting paths
/// that would round-trip lossily via `to_string_lossy()` (replacement
/// char U+FFFD). Storing such paths in TOML would corrupt them on read.
fn path_to_utf8_string(p: &Path) -> Result<String, RootEditError> {
    p.to_str()
        .map(str::to_owned)
        .ok_or_else(|| RootEditError::NonUtf8Path(p.to_path_buf()))
}
...
pub fn add_root_at(
    ...
    // New path is a parent of some existing roots: collect children
    // (for the return value) and remove them from the array in place so
    // that `toml_edit` preserves whitespace / comments on surviving
    // entries. `arr.clear()` + re-push would strip decor.
    let children: Vec<PathBuf> = current
        .iter()
        .filter(|p| is_subpath(p, &canonical))
        .cloned()
        .collect();
    // Reject non-UTF-8 before we mutate the doc so we fail cleanly.
    let canonical_str = path_to_utf8_string(&canonical)?;
    arr.retain(|v| match v.as_str() {
        Some(s) => !is_subpath(&PathBuf::from(s), &canonical),
        None => true,
    });
    arr.push(canonical_str);
...
pub fn remove_root_at(config: &Path, grant_id: &str, target: &Path) -> Result<bool, RootEditError> {
    ...
    // Accept either literal stored string OR canonicalized form. Both
    // comparison strings must be valid UTF-8 — non-UTF-8 input is a
    // programmer error and we surface it explicitly instead of silently
    // mangling via `to_string_lossy`.
    let canonical = std::fs::canonicalize(target).ok();
    let target_str = path_to_utf8_string(target)?;
    let canonical_str = match canonical.as_ref() {
        Some(p) => Some(path_to_utf8_string(p)?),
        None => None,
    };

    let mut removed = false;
    // In-place retain preserves trailing whitespace / comments on the
    // surviving entries; `arr.clear() + loop push` would wipe decor.
    arr.retain(|v| match v.as_str() {
        Some(s) => {
            let hit = s == target_str.as_str()
                || canonical_str.as_deref() == Some(s);
            if hit {
                removed = true;
                false
            } else {
