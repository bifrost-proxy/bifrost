...
impl FileAccessPolicyStore {
    ...
    pub(crate) fn from_raw(cfg: RawConfig) -> Self {
        ...
        for mut g in cfg.grants {
            ...
            // Extend, do not replace: `new_readonly` seeds `policy.denies`
            // with a safe built-in list (.git/.ssh/.env*/id_rsa*/*.pfx/
            // *.p12/Library/Keychains/.npmrc/.netrc). Operator-provided
            // denies must compound on top of that baseline, otherwise a
            // grant that happens to add a single extra deny would silently
            // strip all the built-in secret-path protections.
            if !g.denies.is_empty() {
                policy.denies.extend(g.denies);
            }
        ...
        let default_policy = cfg.default.map(|d| {
            ...
            // See comment above: extend, do not replace.
            if !d.denies.is_empty() {
                policy.denies.extend(d.denies);
            }
    ...
    pub fn resolve(
        ...
        // 1. Exact grant_id. Skip this tier entirely when the caller
        // passed an empty string — otherwise a grant entry that happens
        // to carry `match.grant_id = ""` (or the empty result of a
        // missing fold) would spuriously match every unauthenticated
        // call and leak its policy.
