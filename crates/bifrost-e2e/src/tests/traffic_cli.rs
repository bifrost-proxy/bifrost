...
pub fn get_all_tests() -> Vec<TestCase> {
    ...
    TestCase::standalone(
        "traffic_cli_get_table_format",
        "Validate traffic get with Table format renders without panic and includes header comparison",
        "traffic",
        || async move {
            let port = portpicker::pick_unused_port().ok_or("Failed to pick unused port")?;
            let mock = HttpbinMockServer::start().await;
            ...
            let _ = client
                .post("http://httpbin.org/post", r#"{"test":"table"}"#)
                .await;
            tokio::time::sleep(Duration::from_millis(200)).await;
    ...
    TestCase::standalone(
        "traffic_cli_get_compact_format",
        "Validate traffic get with Compact format renders without panic",
        "traffic",
        || async move {
            let port = portpicker::pick_unused_port().ok_or("Failed to pick unused port")?;
            let mock = HttpbinMockServer::start().await;
