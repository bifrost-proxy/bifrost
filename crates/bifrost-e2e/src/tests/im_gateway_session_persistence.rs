...
pub fn get_all_tests() -> Vec<TestCase> {
    vec![
        TestCase::standalone(
            ...
            let data_dir =
                std::env::temp_dir().join(format!("bifrost_e2e_im_gateway_resume_{port}"));
            ...
            let first_marker = "RESTART_RESUME_FIRST_E2E";
            let second_marker = "RESTART_RESUME_SECOND_E2E";
            let fresh_marker = "RESTART_RESUME_FRESH_AFTER_RESET_E2E";

            let first_response = client
                .post(format!("{base}/agent/chat"))
                .json(&serde_json::json!({
                    "session_key": "restart-resume-e2e",
                    "message": first_marker,
                ...
                .send()
                .await
                .map_err(|e| format!("POST first agent chat failed: {e}"))?;
            assert_status(&first_response, 200)?;

            let state_path = data_dir.join("agent/im_gateway/session_state.json");
            if !state_path.exists() {
                return Err(format!(
                    "Expected persisted IM session state at {}",
                    state_path.display()
                ));
            }
            ...
            let status_response = client
                ...
                .send()
                .await
                .map_err(|e| format!("POST restored status failed: {e}"))?;
            assert_status(&status_response, 200)?;
            let status_json: serde_json::Value = status_response
                .json()
                .await
                .map_err(|e| format!("parse restored status response failed: {e}"))?;
            ...
            let second_response = client
                ...
                .json(&serde_json::json!({
                    "session_key": "restart-resume-e2e",
                    "message": second_marker
                }))
                .send()
                .await
                .map_err(|e| format!("POST second agent chat failed: {e}"))?;
            ...
            let second_request = mock
                ...
                .last()
                .cloned()
                .ok_or_else(|| "mock did not receive second chat request".to_string())?;
            if !request_messages_contain(&second_request, first_marker)
                || !request_messages_contain(&second_request, second_marker)
            {
                return Err(format!(
                    "Expected restart-restored request to contain first and second markers, got: {second_request}"
                ));
            }
            ...
            let reset_response = client
                ...
                .send()
                .await
                .map_err(|e| format!("POST reset failed: {e}"))?;
            ...
            let fresh_response = client
                ...
                .json(&serde_json::json!({
                    "session_key": "restart-resume-e2e",
                    "message": fresh_marker
                }))
                .send()
                .await
                .map_err(|e| format!("POST fresh agent chat failed: {e}"))?;
            ...
            let fresh_request = mock
                ...
                .last()
                .cloned()
                .ok_or_else(|| "mock did not receive fresh chat request".to_string())?;
            if request_messages_contain(&fresh_request, first_marker)
                || request_messages_contain(&fresh_request, second_marker)
                || !request_messages_contain(&fresh_request, fresh_marker)
            {
                return Err(format!(
                    "Expected /reset to clear persisted history before fresh chat, got: {fresh_request}"
                ));
            }
                        ...
                        async move {
                            ...
                            let response = json!({
                                "choices": [{
                                    "message": {
                                        "role": "assistant",
                                        "content": "restart resume mock ok"
                                    },
                                    "finish_reason": "stop"
                                }],
                                "usage": {
