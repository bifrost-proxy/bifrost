...
fn windows_msi_args_force_per_user_install_and_write_log() {
    ...
    let install_dir = PathBuf::from(r"C:\Users\eden\AppData\Local\Bifrost");
    let log = PathBuf::from(r"C:\Users\eden\AppData\Local\Temp\bifrost-msi.log");
    let args = windows_msi_install_args(&package, &install_dir, &log, WindowsMsiScope::PerUser);
    let args = args
        .iter()
    ...
    assert!(args
        .iter()
        .any(|arg| arg == &format!("INSTALLDIR={}", install_dir.display())));
...
fn windows_machine_msi_args_preserve_machine_scope_and_quote_paths() {
    ...
    let install_dir = PathBuf::from(r"C:\Program Files\Bifrost");
    let log = PathBuf::from(r"C:\Users\eden\AppData\Local\Temp\bifrost msi.log");
    let args = windows_msi_install_args(&package, &install_dir, &log, WindowsMsiScope::Machine);
    let rendered = args
        .iter()
        .map(|arg| arg.to_string_lossy().to_string())
        .collect::<Vec<_>>();
...
#[test]
fn windows_desktop_install_transaction_skips_restore_when_installer_changed_nothing() {
    let temp = tempfile::tempdir().expect("tempdir");
    let install_dir = temp.path().join("Bifrost");
    let app = install_dir.join("bifrost-desktop.exe");
    fs::create_dir_all(install_dir.join("resources")).expect("create old install");
    fs::write(&app, "old-app").expect("write old app");
    fs::write(install_dir.join("resources/sidecar.dll"), "old-sidecar").expect("write old sidecar");

    let error = run_windows_desktop_install_transaction::<()>(&install_dir, || {
        Err(BifrostError::Config("invalid MSI".to_string()))
    })
    .expect_err("unchanged failed install still reports the installer error");

    assert!(error.to_string().contains("invalid MSI"));
    assert!(error.to_string().contains("previous desktop app unchanged"));
    assert_eq!(fs::read_to_string(&app).expect("read app"), "old-app");
    assert_eq!(
        fs::read_to_string(install_dir.join("resources/sidecar.dll")).expect("read sidecar"),
        "old-sidecar"
    );
}
...
#[cfg(unix)]
#[test]
fn windows_desktop_tree_comparison_covers_type_size_content_and_symlink_differences() {
    use std::os::unix::fs::symlink;

    let temp = tempfile::tempdir().expect("tempdir");
    let left = temp.path().join("left");
    let right = temp.path().join("right");
    fs::create_dir_all(left.join("nested")).expect("create left tree");
    fs::create_dir_all(right.join("nested")).expect("create right tree");
    fs::write(left.join("nested/file"), b"same").expect("write left file");
    fs::write(right.join("nested/file"), b"same").expect("write right file");
    assert!(paths_have_same_contents(&left, &right).expect("compare equal trees"));

    fs::write(right.join("nested/file"), b"longer").expect("write different length");
    assert!(!paths_have_same_contents(&left, &right).expect("compare file lengths"));
    fs::write(right.join("nested/file"), b"diff").expect("write same-length difference");
    assert!(!paths_have_same_contents(&left, &right).expect("compare file bytes"));

    fs::write(right.join("extra"), b"extra").expect("write extra entry");
    assert!(!paths_have_same_contents(&left, &right).expect("compare directory entries"));
    fs::remove_file(right.join("extra")).expect("remove extra entry");
    fs::remove_file(right.join("nested/file")).expect("remove right file");
    fs::create_dir(right.join("nested/file")).expect("replace file with directory");
    assert!(!paths_have_same_contents(&left, &right).expect("compare entry types"));

    let left_link = temp.path().join("left-link");
    let right_link = temp.path().join("right-link");
    symlink(&left, &left_link).expect("create left symlink");
    symlink(&right, &right_link).expect("create right symlink");
    assert!(!paths_have_same_contents(&left_link, &right_link).expect("compare symlinks"));
    assert!(paths_have_same_contents(&temp.path().join("missing"), &right).is_err());
}
