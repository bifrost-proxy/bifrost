...
Bifrost IM Gateway 的 Agent 通道会在收到用户 IM 消息后触发 Agent loop：模型可能连续调用 shell / read / write 等工具、更新 plan、输出思考、最终给出回答。这一过程可能持续数十秒到数分钟。如果只在 loop 结束后一次性回复消息，用户在等待期间没有任何反馈，无法判断“Agent 是否还在工作、工作到哪一步、下一次输入会不会被吞掉”。

需要一张“进度卡”：在 loop 开始时立刻发出、随 Agent 事件更新工具/plan/思考/最终输出/状态、loop 结束后固化为终态卡片留在历史里。飞书 CardKit 支持 `streaming_mode=true` 的流式卡片，可以在同一 `card_id` 上局部更新组件；其它 IM 平台（微信、Webhook 等）能力较弱，本设计需要 provider-neutral 抽象让每个平台按能力落地。
...
- 卡片区块包含：
  ...
  - 思考过程：独立模块，直接展示最后一次正在输出的完整思考文本，不需要展开。
- 过程时间线（thinking / tool / status）在 `agent_process_panel` 中按序渲染，超长任务只保留最近 30 次工具调用并显示已省略数量。
...
- 真实 Agent loop 触发 progress card 更新：肉眼观察卡片持续更新，工具面板出现、plan 变化、思考文本、token 消耗、guide/queue 状态区提示、终态输出。
...
- `agent_thinking_panel` — 过程思考直接显示（普通 markdown）。
...
- `run_progress_event_coalescer`：对 status 类事件按 300ms 合并刷新；工具、计划、标题、过程文本、最终输出和结束事件立即刷新；Feishu session 继续按 section fingerprint 过滤未变化模块。
...
- 组件 element_id 固化：output / plan / tool / thinking / process / status。
...
- 过程文本使用普通 `agent_thinking_panel` markdown 元素，默认直接展示最后一次 `AssistantDelta` 的完整内容；`agent_process_panel` 时间线条目带 `ap_t` / `ap_td` / `ap_tg` element id。长任务只保留最近 30 次工具调用的可见后缀，顶部摘要写明前面已省略的调用次数。
