...
本设计的核心约束是“一个 IM Agent turn 对应一张进度卡片”。收到 IM 消息后立即创建并发送 CardKit streaming card；Agent loop 生命周期内更新这张卡片的固定组件。运行中收到 guide/queue 用户消息时，旧的 Running 卡片会 best-effort 撤回并在最新用户消息下方重发新卡片；当 queue 消息在上一轮仍处于 Running/收尾状态时被消费为下一轮用户消息，也采用同样的撤回重发逻辑，后续进展更新新卡片。loop 结束后卡片进入 Finished/Failed 终态，终态历史卡片必须保留，后续独立新消息只能新建下一张卡片，不能撤回已完成卡片。
...
- guide 消息进入时更新 Running 卡片的底部 guide 状态，并通过撤回重发把进度卡片移动到最新用户消息之后。
- queue 消息进入或删除时更新 Running 卡片的底部排队状态，并通过撤回重发把进度卡片移动到最新用户消息之后；终态历史卡片不参与撤回重发。
...
Feishu V1 使用 `StreamingCard`，通过 CardKit 创建 card entity，再用 IM send API 发送 card entity。执行中更新固定元素内容和工具折叠面板，结束时关闭 `streaming_mode`。guide/queue 触发时更新 progress snapshot 后撤回旧卡片并重发新卡片；如果找不到活跃 progress session，才回退发送普通确认消息。
...
IM message received
  ...
  -> queued message becomes next turn
       -> if previous card is still Running, DELETE /im/v1/messages/{old_message_id} best-effort
...
- `handle_busy_message` 在 guide / queue / remove queue 成功后通知 progress session 更新 snapshot。只有当前 progress session 仍处于 `Running` 时才 best-effort 撤回旧进度卡片并在最新用户消息下方发送新卡片；如果 progress session 已经 `Finished` / `Failed`，不能撤回历史卡片，必须返回未更新并回退到普通确认消息或下一轮新卡片。
- `process_agent_chat` 在下一轮启动 progress session 时优先调用 `repost_existing`：只有旧卡片仍处于 `Running` 时才用飞书消息撤回 API best-effort 撤回旧卡片消息，再重置 snapshot、创建新的 CardKit card entity 并发送新 interactive 消息；如果旧卡片已终态，`repost_existing` 返回 false，上层直接 `start_feishu` 新建下一张进度卡片。撤回失败只记录 warn，不阻断下一轮新卡片发送；新卡片发送失败则返回错误并由 registry 记录 warn。
...
- guide/queue 更新会撤回旧消息并发送新 card entity，使进度卡片移动到最新用户消息下方。
- queue 消息被消费为下一轮时，如果旧 progress session 仍处于 Running，progress registry 应调用 Feishu 撤回旧消息 API，并创建新的 card entity / message；撤回失败不得阻断新卡片发送。
- 已 Finished/Failed 的历史卡片收到新一轮消息时，progress registry 不得调用撤回 API，不得改写旧 snapshot；上层应新建下一张 progress card。
...
- 新增或复用 IM Gateway mock inbound E2E，验证 queue 消息进入下一轮时触发 `repost_existing`，后续 progress event 指向新卡片；同时验证终态卡片不会被撤回，turn-end 窗口消息会进入 guide/queue。
...
- 更新 `human_tests/im-gateway-agent.md`，新增流式进度卡片用例：
  ...
  - guide 消息：Running 卡片撤回重发到最新用户消息之后，并展示待处理 guide。
  - queue 消息：Running 卡片撤回重发到最新用户消息之后，并展示排队数量。
  - queue 消息成为下一轮：Running 旧卡片撤回，新卡片出现在最新用户消息下方，后续进展更新新卡片。
...
- 目标复核：确认四区卡片、Running 卡片撤回重发、终态卡片保留、turn-end 消息不丢失和 provider-neutral 入口都已落地。
