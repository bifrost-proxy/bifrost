...
需要一张“进度卡”：在 loop 开始时立刻发出、随 Agent 事件更新工具/plan/最终输出/状态、loop 结束后固化为终态卡片留在历史里。模型的流式中间正文不属于过程信号，避免与最终输出重复。飞书 CardKit 支持 `streaming_mode=true` 的流式卡片，可以在同一 `card_id` 上局部更新组件；其它 IM 平台（微信、Webhook 等）能力较弱，本设计需要 provider-neutral 抽象让每个平台按能力落地。
...
- 过程时间线（tool / status）在 `agent_process_panel` 中按序渲染，超长任务只保留最近 30 次工具调用并显示已省略数量；`AssistantDelta` 和运行中的 `AssistantFinal` 不渲染。
...
- 流式 assistant 正文只用于 Runner 内部事件与最终响应收敛，不进入用户可见的过程区。
...
pub struct ImAgentProgressSnapshot {
    ...
    pub timeline: Vec<TimelineItem>,    // tool / status；assistant 正文不写入
...
- `agent_process_panel` — 过程时间线（tool / status），条目 element id 前缀 `ap_t` / `ap_td` / `ap_tg`，便于按 section fingerprint 增量更新。
...
- `run_progress_event_coalescer`：对 status 类事件按 300ms 合并刷新；工具、计划、标题、最终输出和结束事件立即刷新；assistant 正文增量在 progress snapshot 边界过滤；Feishu session 继续按 section fingerprint 过滤未变化模块。
...
- `AssistantDelta` 与运行中的 `AssistantFinal` 不进入 progress snapshot；`agent_process_panel` 只展示工具与可读状态，条目带 `ap_t` / `ap_td` / `ap_tg` element id。长任务只保留最近 30 次工具调用的可见后缀，顶部摘要写明前面已省略的调用次数。
...
- 新增 IM Agent streaming card 回归：验证 progress renderer 输出 JSON 2.0 streaming card、固定组件 id、可选 plan/tool 模块、assistant stream 隐藏、最终正文只出现一次、工具耗时和 guide/queue 状态区。
