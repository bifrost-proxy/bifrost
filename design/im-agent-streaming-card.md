...
- Feishu CardKit 整卡更新遇到 `code=300305 element exceeds the limit` 时应发送新 card entity，并把后续 progress event / final close 指向新 `card_id`；旧卡片冻结失败不影响新卡片继续更新。
