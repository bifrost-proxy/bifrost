...
- 展开支持嵌套引用；如果引用不存在或出现循环引用，入口规则跳过解析并记录明确错误。
- 主代理规则热加载、临时端口绑定、Replay 规则解析和 Rules validate API 复用同一展开语义。
...
- 单元测试：
  - `rule::references` 覆盖引用识别、原位展开、嵌套引用、缺失引用、循环引用。
  ...
  - `load_stored_rules_skips_entry_with_missing_reference` 验证缺失引用不会生成运行时规则。
  - tokenizer 测试验证 `@规则名称` 被标记为 reference token。
- E2E 测试：
  - `e2e-tests/tests/test_rule_references.sh` 通过真实 Admin API 创建 disabled shared、预置 disabled 组 shared 和 enabled entry，验证 validate API 解析为 3 条规则，并通过真实代理请求确认两个 shared 规则里的请求头生效。
  - `web/tests/ui/admin-rules-values.spec.ts` 验证 validate API 缺失引用错误、reference candidates 自动检索、Monaco 模糊补全和 Rules 编辑器点击 `@规则` / `@组名/规则名` 展开详情。
- 真实场景测试：
  - 更新 `human_tests/webui-rules.md`，新增 `TC-WRU-45`，覆盖亮色和暗色主题下的 `@` 展开、收起、详情内容、组规则引用、候选检索、模糊补全和解析回归。
