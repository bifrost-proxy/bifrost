...
- 在 ByteDance 内网自动检测可用 provider, 检测通过后自动推荐或配置 `https://bifrost.example.com`。
...
The `bytedance-internal` managed provider wraps `bifrost-server` type with canonical URL `https://bifrost.example.com`.

Detection:

1. Probe `GET https://bifrost.example.com/v4/sso/check`.
