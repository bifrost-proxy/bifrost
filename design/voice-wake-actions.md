...
Voice Wake Actions 为本机语音触发全局键盘指令能力。产品目标不是暴露 profile/binding 等内部对象，而是提供一条短流程：用户先录入一段唤醒音频，后端 ASR 从样本中识别唤醒文本，用户在快捷键输入框里直接按下组合键，点击 Start Listening 后由 Bifrost 后台进程持续监听并触发真实 `key_press` 动作。
...
- WebUI 在 ASR 页面提供 Voice Wake Actions 面板，支持录入唤醒音频、展示只读唤醒文本、用输入框捕获全局快捷键、保存动作、启动/停止后台监听和事件查看；WebUI 不展示 dry-run 概念，后台监听命中后真实执行。
- 浏览器只用于一次性配置与样本采集；Start Listening 后由 Bifrost 主服务拉起独立 worker 进程完成麦克风采集、ASR、声纹识别、匹配和按键触发，页面关闭不影响后台 listener，主服务 stop 后 worker 必须跟随退出。
...
bifrost ai voice wake bind-audio ./wake.wav --voiceprint-profile-id speaker_eden
...
`bind-audio` 是 CLI 的一条完整配置路径：传入一次唤醒音频样本，CLI 调用后台 ASR 识别唤醒文本，绑定已有 Speaker Diarization 声纹 profile；如果不传 `--key`，终端进入快捷键捕获模式，用户直接按下要绑定的组合键。自动化或无 TTY 环境可以显式传 `--key space --modifiers cmd`。测试环境保留隐藏 `--phrase` 覆盖参数，用于无模型回归；面向用户的正式路径不展示也不依赖手工输入唤醒词。
...
- `Wake Audio`：通过浏览器麦克风录入一次唤醒音频样本，停止录音后调用后端 ASR 识别。
- `Wake phrase`：紧跟在录音样本下方，只读展示后端 ASR 从样本中识别出的唤醒文本；用户不能手动编辑，避免配置文本和实际声音不一致。
...
- `Start Listening`：与开关使用同一门禁，调用 `POST /api/voice/wake/listener/start`，Bifrost 主服务拉起独立 worker 进程；worker 用本机麦克风采集音频、送本机 ASR 服务识别、匹配 binding phrase，再用同一音频块调用现有 sherpa-onnx speaker embedding 识别已有声纹；只有文本和声纹都命中且分数达到阈值后才执行真实 macOS `key_press`。
...
- KWS/ASR 文本侧：当前 listener 用后端 ASR 文本匹配 binding phrase；后续可替换为 sherpa-onnx open vocabulary KWS，但 WebUI 流程不变。
...
- 双门禁：`normalized transcript contains normalized phrase` 且 `identified_profile_id == VoiceWakeProfile.voiceprint_profile_id` 且 `confidence >= binding.speaker_threshold` 才触发动作。

因此 Voice Wake 不允许用户手动录入或编辑声纹标识，也不在本模块里重新采集声纹；唯一声纹来源是现有声纹录入模块。

## 依赖项

- 复用现有 ASR 服务转写样本和后台 listener 音频切片。
...
- `cargo test -p bifrost-cli extract_wake_phrase_from_sse --lib`：验证 ASR 结果清理和 worker 父进程守护。
...
- `e2e-tests/tests/test_voice_wake_actions.sh`
  ...
  - 通过 CLI `bind-audio` 从音频样本创建关联已有 voiceprint profile 的 Voice Wake profile 和 trigger phrase 到 key press 的 binding；
  - 通过 CLI `listener start` 验证启动后台监听入口；
  - 通过 CLI dry-run trigger；
  - 通过 listener mock source 验证后台 listener start 只有 ASR transcript 和 speaker profile 都命中才记录事件；
...
- `web/tests/ui/voice-wake-actions.spec.ts`
  ...
  - 验证录音入口、只读唤醒词在录音区域下方、已有声纹选择、快捷键输入框捕获和 Voice command 开关；
...
- `human_tests/voice-wake-actions.md`
  ...
  - 覆盖 WebUI 录音入口、后端 ASR 识别唤醒文本、选择已有声纹、快捷键绑定、Start Listening、后台监听触发真实执行；
...
- 复核用户目标：录音样本生成只读唤醒文本、快捷键绑定、后台启动监听、说话触发真实按键是否可见且可测。
...
- 复查第 1 轮修复后的 diff。
- 复查 human_tests 索引、API/CLI 输出、WebUI 不展示 dry-run、不允许手动编辑唤醒词且监听请求进入后台 listener。
- 复跑受影响测试和最终启动验证。
