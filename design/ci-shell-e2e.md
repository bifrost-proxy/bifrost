...
- 静态解析 `e2e-tests/run_all_tests_parallel.sh`，断言存在纯 bash `result_has_status`，Windows 下 `loop_sleep` 默认为 `BIFROST_E2E_WINDOWS_POLL_INTERVAL:-1`，主循环用 `result_has_status "$rf" || ! kill -0 ...` 回收 suite，并在 `BIFROST_E2E_RULE_RUNNER_TIMEOUT` 到点时写入未完成 fixture 的失败结果后退出。
