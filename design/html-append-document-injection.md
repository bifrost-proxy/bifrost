...
预期是匹配 `https://www.qq.com`、`https://www.qq.com/` 和 `https://news.qq.com/rain/a/20260428A07D9U00` 这类单级子域名 HTML 页面，并执行 `htmlAppend`。旧 `WildcardMatcher` 对带尾部 `/` 的通配域名会把规则编译成“域名后已有 `/`，再追加一个可选的 `/.*`”的正则，导致子路径请求需要两个连续斜杠才会匹配；无尾斜杠 URL 也会漏匹配。修复后，通配域名显式写根路径时按路径前缀语义处理，尾部 `/` 同时覆盖空路径、根路径和任意子路径。
...
- `WildcardMatcher` 对带路径的通配域名不再盲目追加 `(/.*)?`。当规则路径以 `/` 结尾时，生成的正则允许 URL 无路径、根路径或任意子路径；当规则路径不以 `/` 结尾时，继续保持路径前缀语义。
...
- `test_prefix_wildcard_with_root_path_matches_subpaths`：验证 `*.qq.com/` 匹配 `https://www.qq.com`、`https://www.qq.com/`、`https://news.qq.com/rain/a/20260428A07D9U00`，且不匹配多级子域名或非 qq 域名。
- `test_parse_wildcard_root_path_pattern`：验证 parser 将 `*.qq.com/` 识别为通配匹配器，并保持上述匹配语义。
...
- `matcher_wildcard_root_path_html_append`：通过真实代理请求验证 `*.qq.com/ htmlAppend://(...)` 对 `www.qq.com/` 与 `news.qq.com/rain/...` 均命中并注入 HTML。
...
- 新增 `human_tests/proxy-rules-advanced.md` 的 `TC-PRA-29E`，覆盖 `*.qq.com/ htmlAppend://{vconsole-inject}` 对根路径和子路径页面均命中，验证真实用户感知的 HTML 注入结果。
...
- 按 `human_tests/proxy-rules-advanced.md` 执行 `TC-PRA-25`、`TC-PRA-29A`、`TC-PRA-29B`、`TC-PRA-29D` 与 `TC-PRA-29E`
- `cargo fmt --all -- --check`
