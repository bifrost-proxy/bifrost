...
- 系统代理 readiness 探针必须读取到 HTTP 状态行后再判断，不假设单次 TCP `read` 能拿到完整 `HTTP/1.1 2xx`；空响应、非 2xx 或超时均视为未就绪。
...
会话恢复仍保留用户消息、最终助手消息、紧凑工具摘要和子 Agent 终态摘要。外部执行器的目标、计划与运行中进度只供实时 UI 消费，新运行不再写入 durable history；旧版本已保存的事件仍可兼容读取。原始超长工具输出不再作为审计日志保存；需要完整构建日志的场景应由对应 CLI/CI 系统承担，而不是 Agent 会话目录。
