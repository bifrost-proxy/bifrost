...
| QPS | 最近 1 秒 request event 窗口 | 每个 request event 进入环形事件队列，`get_current` 按最近 1000ms 统计，不依赖 UI 轮询间隔。 |
...
| Upload/download rate | 最近 1 秒 byte event 窗口 | 每次 bytes sent/received 入账时记录事件，`get_current` 按最近 1000ms 统计。 |
