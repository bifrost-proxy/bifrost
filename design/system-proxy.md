...
- 收到 SIGTERM/SIGINT/SIGHUP 时，前台和 daemon 分支都会优先调用 `SystemProxyManager::restore`，再清理代理 listener、后台任务、ASR/浏览器/Agent worker 等资源。关机/重启前的短窗口优先恢复 Wi-Fi/Web 代理，减少系统重启后仍绑定到已消失 Bifrost 端口的风险。
