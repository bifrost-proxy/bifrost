...
- CLI 与 Admin API/Web UI 均支持安装/卸载/状态查询 cleanup LaunchDaemon。`GET /api/proxy/system/launchd` 返回 `installed_version`、`current_version`、`needs_upgrade`、plist 路径和加载状态；`PUT /api/proxy/system/launchd` 通过 GUI 授权安装或卸载。Web UI Settings Proxy 页提供 `Boot/Shutdown Cleanup` 开关，版本不一致时展示 needs-upgrade 状态，用户打开开关会重新安装当前版本；该开关是显式管理入口，不是系统代理启用时自动保护安装的唯一入口。
...
- 必须真实执行一次 macOS GUI 授权安装/卸载验证：启动 Bifrost 并启用系统代理后确认服务先 ready，再观察授权弹窗；运行中服务通过 Admin API/Web UI 打开系统代理时同样观察到 LaunchDaemon 自动检查/授权安装；用户授权后确认 `launchctl print system/com.bifrost.system-proxy-cleanup` 成功，Web UI 开关可卸载并再次安装，版本一致时再次启动不重复弹授权。
