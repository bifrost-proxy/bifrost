...
- CI shell E2E shard 使用 `SKIP_BUILD=true` 复用预构建 `BIFROST_BIN`，避免在 shard 内重新编译；`test_security_hardening_functional.sh` 在该模式下跳过 `cargo run -p bifrost-e2e` 子步骤，由 dedicated bifrost-e2e job 与本地 full wrapper 覆盖 Admin/Remote Invoke runner 路径，shell shard 继续覆盖 Sync、BP parser、DevTools 功能回归。
- `test_breakpoint_performance_guard.sh` 在 `SKIP_BUILD=true` 时复用预构建 bifrost 二进制，避免 Mac shell shard 因重新构建 debug binary 超时。
