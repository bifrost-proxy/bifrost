...
- 采样线程每 1 秒刷新网络速率；CPU 与内存每 3 秒刷新一次并复用最近值。
- 使用同一个 `sysinfo::System` 与 `sysinfo::Networks` 实例循环刷新，避免 CPU 与网络差值类指标在重建实例时失真。
- 网络速率按接口保存平台累计字节计数，下一次采样用同一接口的累计差值除以 `Instant` 单调时钟间隔，转换为 bytes/sec 后格式化。
- 首次采样、网卡新增/移除或计数回退时，该接口当前拍不参与速率计算，避免显示虚高尖峰。

### 性能与实时性权衡

刷新频率不做成用户可见配置，默认网络 1 秒、CPU/内存 3 秒。理由：

- 网速变化更快，1 秒刷新更符合用户对实时吞吐的感知；CPU/内存变化相对慢，3 秒刷新足够判断当前系统是否繁忙。
- 采样只在 tray helper 本地执行，不访问 Admin API，不阻塞主代理进程。
- 当 `show_system_stats=false` 时，线程只读取配置并清空菜单状态，不执行 `sysinfo` CPU/内存/网络采样；再次开启时重置网络累计基线，避免把关闭期间的字节变化折算成当前速率。
...
若后续真实用户机器上看到明显 CPU 增高，优先把网络刷新间隔调高到 2 秒，或把 CPU/内存刷新间隔调高到 5 秒；只有存在明确用户需求时再增加 Settings 中的刷新频率选项。
...
- 新增/更新 Playwright 覆盖 `Settings -> Tray` tab、两个开关可见与保存，并在桌面与移动端 viewport 截图确认图标、文案、开关没有遮挡、错位或横向溢出。
