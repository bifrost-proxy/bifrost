...
7. progress card 运行中继续展开任务计划，但执行过程默认折叠；折叠栏上方的“最新进展”只展示最新公开解释，当前工具与本轮三态计数并入过程标题单行展示。进入 `Finished` 或 `Failed` 后移除“最新进展”，计划、过程和结论板块全部设置 `expanded=false`。最终正文仍保留在原卡的“最终结论/失败结论”折叠板块中，作为独立终态卡发送失败时的可追溯兜底。

### Codex 本地文件链接中的源码位置后缀

Codex 会用 `[方案.md](/absolute/path/方案.md:1)` 或 `[notes](./notes.txt:12:3)` 表达“文件 + 行号/列号”。飞书附件提取不能把末尾数字位置当成文件名的一部分：

1. 先按原始目标解析本地路径；若该路径真实存在，必须原样使用，从而兼容合法包含冒号和数字的文件名。
2. 仅当原始路径不存在时，最多从右侧剥离两个纯数字段，分别兼容 `:line` 与 `:line:column`。
3. 只有剥离后的候选是现存文件时才采用；否则保留原始解析结果和既有失败语义，不能把普通冒号路径误改写。
4. 归一化同时适用于绝对路径、相对工作目录路径和 `file://` 路径；远程 URL 仍不进入本地附件逻辑。
5. 附件类型判断使用最终解析出的路径，因此 `.md:1` 能恢复为 `.md` 并进入既有 Markdown 文件上传流程。
...
- shell E2E：成功总结包含中文 Markdown 文件的 `:1` 链接，断言 `/im/v1/files` 上传的真实文件名为 `方案.md` 而不是 `方案.md:1`，随后发送对应 `msg_type=file` 消息。
