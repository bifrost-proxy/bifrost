...
- 交互卡片会提取标题、Markdown、正文和折叠区等可见文本，忽略按钮、URL 和动作 value。CardKit 首次只返回 `card_id` 时再读取默认消息表示，获得用户当前可见内容。被直接引用的图片与文件会按 `design/feishu-referenced-message-attachments.md` 使用原消息 ID 下载到当前会话附件目录并把绝对路径交给模型；未被本轮引用的历史二进制资源不会自动补拉，音视频暂不在本能力范围内。
