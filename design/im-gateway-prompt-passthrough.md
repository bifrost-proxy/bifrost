...
外部 Runner 的 Agent Base / Developer / User Instructions 与 Runner Instructions 都为空时，Bifrost 仍会在每一条消息前自动拼接 `Bifrost Tool Context`。这段内容不是用户配置，而是旧的 `injectBifrostTools` 开关触发的硬编码 prompt；内置 Runner 又默认把该开关设为开启，因此空配置不等于空注入。
...
- Agent 与 Runner 指令全部为空时，Runner 收到的正文必须是通道实际传入的消息，不增加 Bifrost 自带说明。
...
- 指令按 `Base（仅首条） -> Developer -> User -> Runner -> 通道消息` 的顺序组合；空白项被忽略，不生成标题或占位文本。
...
- 使用捕获 stdin 的 mock Runner 验证全空配置时首条和后续消息均原样到达。
...
每一项都执行 trim 后判空。所有指令为空时，不创建额外 section，最终 prompt 只包含原消息及 Runner 输入文件所需的结尾换行。Bifrost 不猜测用户是否需要工具说明；需要额外上下文时，必须通过可见的 Agent 或 Runner 指令显式配置。
...
- `build_prompt`：所有 Instructions 为空且遗留开关为 `true` 时，不包含自动上下文。
- 指令组合函数：首条完整顺序、后续不含 Base、全空返回 `None`、空白项被忽略。
...
1. 空 Agent / Runner 指令、遗留开关为 `true`，断言捕获内容等于原消息。
2. 配置四层指令并连续发送两条同会话消息，断言首条包含 Base，第二条不含 Base，其他层每条存在且顺序正确。
3. 两条消息均断言不包含 `Bifrost Tool Context`。

### human_tests

在 `human_tests/im-gateway-external-cli-chat-gateway.md` 增加空配置透传与新会话 Base 生命周期回归用例，并同步相关索引行。文档更新后立即按步骤执行。

## 覆盖率与交付

本地不运行 `make coverage` 或 `scripts/ci/coverage-all.sh`。业务代码覆盖率由远端 CI 的 `bash scripts/ci/coverage-all.sh --json --gate` 门禁验证。
