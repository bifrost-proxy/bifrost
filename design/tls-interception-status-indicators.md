...
- Tray helper 轮询 Admin API 的 `/api/config/tls`，在 `enable_tls_interception=true` 时渲染带角标的 tray icon。macOS 系统状态标题模式下，在菜单栏统计标题前增加 `TLS` 短标签，避免 bitmap 统计标题覆盖图标角标。
- Tray 下拉菜单中的 System Proxy 从单行勾选项改为 `System Proxy: On/Off/Checking/Pending` 子菜单，子菜单内展示只读状态行和 `Enable/Disable System Proxy` 操作项。
...
- 单元测试：`web/src/components/StatusBar/statusIndicators.test.ts` 覆盖 TLS full/scoped/unknown 状态派生；`crates/bifrost-cli/src/commands/tray/menu.rs` 与 `tray_tests.rs` 覆盖 System Proxy 子菜单和 TLS 状态读取。
- E2E 测试：`web/tests/ui/admin-settings.spec.ts` mock TLS 全局启用，断言状态栏 `Full On`、动画 class 和跳转入口。
- 真实场景测试：`human_tests/tls-interception-status-indicators.md` 覆盖 Web 状态栏亮/暗主题、tray 角标和 tray System Proxy 子菜单。

## Review/Fix/Test 闭环方案

- 第 1 轮复核 Web 状态派生、push 订阅、tray snapshot、icon 角标和 System Proxy 菜单结构，运行前端单元测试、tray 相关 Rust 单元测试和 Web UI E2E。
