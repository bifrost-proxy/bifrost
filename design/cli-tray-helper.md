...
- macOS 菜单栏 `MEM` 使用偏用户感知的即时压力口径，而不是 `total - free` 的物理非空闲口径：采样基于 `vm_statistics64` 的 `internal - purgeable + wired`，排除 file-backed / inactive cache / 可回收缓存，也不把 long-lived compressed store 当成当前活跃消耗，避免把系统缓存或压缩冷页误报成 75%-80% 的真实内存压力；极端情况下扩展页计数字段为空时才降级到非空闲页估算。
