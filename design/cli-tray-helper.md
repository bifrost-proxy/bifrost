...
- macOS 菜单栏 `MEM` 使用偏用户感知的即时压力口径，而不是 `total - free` 的物理非空闲口径：采样基于 `vm_statistics64` 的 `active + wired - purgeable`，排除 inactive / file-backed cache / 可回收缓存，也不把 long-lived compressed store 或冷的匿名页当成当前活跃消耗，避免把系统缓存或压缩冷页误报成 75%-80% 的真实内存压力；如果 active/wired 字段不可用，则先降级到 `internal - purgeable + wired`，极端情况下才降级到非空闲页估算。
...
- 2026-06-21 review 追加修复：确认三项问题真实存在并修复。`poll_system_stats` 不再每秒同步读 `config.toml`；默认路由刷新不再在采样线程同步 `Command::new("route").output()`；native AppKit status item 增加 main-thread only 编译期约束。验证命令：`cargo test -p bifrost-cli system_stats --lib --all-features -- --nocapture` 覆盖 config parse error 保留旧配置、route timeout 和后台 resolver 结果消费；`cargo test -p bifrost-cli native_ --lib --all-features -- --nocapture` 覆盖 native 状态项相关回归；`cargo check -p bifrost-cli --all-features` 与 `cargo clippy -p bifrost-cli --all-targets --all-features -- -D warnings` 通过。
- 2026-06-21 render 热路径二次优化：在最新 `origin/main` 上做同机 60 秒 release 对照采样，baseline 为 `/tmp/bifrost-tray-main-baseline-perf.DzGVtu`，`samples=60 avg_cpu=0.3500 p50=0.2000 p90=1.0000 p95=1.3000 max_cpu=1.5000 avg_rss_kb=68244 rss_delta_kb=2288`；优化分支为 `/tmp/bifrost-tray-render-perf.wmDCJB`，`samples=60 avg_cpu=0.3150 p50=0.2000 p90=0.9000 p95=1.0000 max_cpu=1.1000 avg_rss_kb=68343 rss_delta_kb=2256`。结论：保持 1 秒网络刷新和原有视觉效果，平均 CPU 降约 10%，p95 从 1.3% 降到 1.0%，max 从 1.5% 降到 1.1%；RSS 无显著增长。
