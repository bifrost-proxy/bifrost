...
- 普通列表最多常驻 2,000 条；向上滚动可加载更老记录，向下滚动可继续回到更新记录。
- 筛选仍覆盖数据库完整历史，而不是只匹配当前 2,000 条窗口；筛选结果最多常驻 2,000 条，并可继续加载更老命中项。
...
- 页面休眠/恢复或 WebSocket 重连时，补推流量按最多 500 条一批发送，前端待处理队列最多保留 2,000 条记录，不能因瞬时 backlog 重回无界内存。
...
- 单元测试验证普通窗口在向前/向后合并后不超过 2,000 条，淘汰方向正确且游标单调。
...
| 模式 | 数据来源 | 常驻上限 | 历史范围 |
| --- | --- | ---: | --- |
| 普通 Network | `/traffic/updates` + `/traffic` 双向分页 + Push | 2,000 | 通过前后游标连续浏览全部历史 |
| Network 筛选 | `/traffic` compact 分页扫描，浏览器只保留命中项 | 2,000 | 每次筛选从最新向最老扫描全部历史；之后继续加载旧命中项 |
...
- 顶部接近阈值：以当前最老 `sequence` backward 拉 500 条，前插后从最新侧裁剪到 2,000 条。
- 底部接近阈值且 `hasNewer=true`：以当前窗口最新 `sequence` forward 拉 500 条，后插后从最老侧裁剪到 2,000 条。
...
最新窗口最终上限为 `min(2_000, server_total)`；历史窗口不插入新记录，但会按 `oldest_sequence` 清除已经被服务端淘汰的行，并保留 `hasNewer` 以便向新方向恢复。
...
2. WebSocket 单条 `traffic_delta` 的 `inserts + updates <= 500`；连接级发送队列继续保持 64 条容量。重连时只读取最新 2,000 条服务端窗口中晚于客户端游标的记录，再拆成最多 500 条一包连续发送；这样即使游标已经落在淘汰区间，也能收敛到当前尾部而不会回放全部历史；
3. Web 端把同一帧内收到的 delta 合并到有界待处理批次：new/update 各自 ID 去重并最多 2,000 条，只调度一个 timer/RAF；提交后普通窗口和 Map 再执行 2,000 条硬裁剪。
...
5. 用户向上滚动时从已扫描游标继续，累计结果超过 2,000 时从较新侧裁剪；
...
- `records.length <= 2_000`。
- `recordsMap.size <= 2_000`，且 key 集合与 `records` 完全一致。
- `pendingBatch.newRecords.length <= 2_000`、`pendingBatch.updatedRecords.length <= 2_000`；服务端单条 delta 最多 500 条。
- 普通 `records` 与筛选结果均严格按 `sequence` 升序且 ID 去重。
- 单次筛选扫描临时批次 `<= 500`，筛选结果 `<= 2_000`。
...
- 服务端滚动长压测写入 3,000 条真实本地代理请求，断言全部 sequence 落库、最终存量位于 80%–115% 软边界、最老水位前移且完整链路小于 480 秒。UI 休眠恢复用 600 条初始 + 600 条 hidden 写入，既触发服务端滚动又跨越 500 单包边界；断言恢复后单批 Push `<= 500`、DOM loaded count `<= min(2_000, server_total)`、旧淘汰记录不可见、Tab 切换在 3 秒内完成、事件循环最大停顿小于 1.5 秒；若 Chromium 暴露 `usedJSHeapSize`，恢复后必须低于 512 MiB。另以 5,000 条同帧 delta 单元测试覆盖更大的前端合并洪峰。
