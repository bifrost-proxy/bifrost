...
- Search 保持服务端搜索、50 条分页和最多 1,000 条结果的有界语义；搜索执行期间收到的实时新增、完成更新、删除和滚动淘汰必须在 1 秒内反映到结果，不能依赖用户再次点击搜索。
...
Search 首次查询仍扫描完整服务端历史。查询完成后的实时变化不重新扫描全库：前端把 1 秒内的 insert / update ID 去重合并后，调用同一搜索接口的定向 `record_ids` 模式，只让服务端读取并重新匹配这些记录。删除和低于服务端最老水位的结果直接从当前结果移除。若单秒突发超过定向重算上限、发生 reset，或增量批次已经被前端有界队列裁剪，则放弃不完整的 ID 集合并自动重跑一次完整搜索，以准确性优先收敛。
...
- Search 定向重算单次 ID 数 `<= 500`，同一搜索会话最多每秒发起一次实时刷新；返回结果必须只来自请求 ID 集合且仍满足当前关键字、scope 和全部筛选条件。
- Search 实时结果按 `sequence` 降序、ID 去重且 `<= 1_000`；insert、pending → completed、由不命中变为命中、由命中变为不命中、精确删除和滚动淘汰都必须收敛。
...
- Search 实时定向重算失败：保留上一次完整结果，记录本批为不完整并在下一次 1 秒刷新执行完整搜索；关键字、scope 或筛选条件变化会使旧请求 generation 失效，旧响应不得覆盖新条件结果。

## 全面一致性与可靠性测试矩阵

交付门禁同时覆盖普通 Network、左侧服务端统计和 Search，不能只验证首屏样例：

| 维度 | 必测场景 | 一致性断言 |
| --- | --- | --- |
| 无条件 | 无关键字、无 Toolbar、无高级条件、无左侧选择 | 最新窗口严格等于服务端最新 `min(1_000, total)` 条；权威统计等于服务端保留全集 |
| 单项过滤 | method、status、protocol、content type、rule、host/path/url、client IP/app/account、port、domain、空值/非空 | 普通筛选扫描全集；结果 ID 与服务端查询真值集合一致 |
| 组合过滤 | Toolbar + 高级条件 + 左侧维度，含肯定、否定、contains、equals、正则和空值组合 | 条件按既有 AND/OR 语义组合，窗口裁剪不改变命中集合 |
| Search | URL、请求/响应 header、请求/响应 body、WS/SSE scope 与上述筛选组合 | Search 结果只含同时满足内容匹配与过滤条件的记录；历史窗口之外记录仍可找到 |
| 实时变化 | 新增命中/不命中、pending 完成、字段或 body 更新后晋级/降级、删除、retention 水位推进 | 普通列表、筛选列表、Search 和权威统计在最多 1 秒合并周期后收敛，不重复、不漏项 |
| 爆发与重连 | 单连接连续写入 >1,000 条、WebSocket 断开后继续写入、重连游标落后超过补推窗口 | 推送序列单调；单帧 `<=500`、前端队列/窗口 `<=1,000`；超限触发安全重载而非静默丢失 |
| 竞态 | 搜索中切换关键字/scope/筛选，旧请求晚返回；清空与实时推送并发 | generation 隔离旧响应；已删除记录不复活；最终结果匹配最新条件 |
| 性能稳定性 | 重复执行组合矩阵和突发写入，采样查询耗时、事件循环停顿、进程存量 | 无持续增长、无全表按秒重算、无推送风暴；统计读取和定向重算保持有界 |

自动化测试使用带唯一 marker 的真实代理请求建立真值集，并以记录 ID/sequence 比较 API、WebSocket 与 UI，而不是只比较易受并发影响的总数。稳定性门禁至少重复三轮隔离运行；任一轮出现缺失 ID、重复 ID、乱序、超出窗口或超时都视为失败。
