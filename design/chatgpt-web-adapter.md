...
- `stream_handoff` 没有携带可解析 SSE final 时，DOM 默认只能提供 busy/progress 信号，不能独立确认最终态。即使 stop button 暂时消失、composer 恢复可用、页面已经出现正式 `data-message-id`，也必须读取同一 conversation 的 backend detail，并沿 `current_node` 当前分支确认本轮 assistant 消息同时满足 `status=finished_successfully`、`end_turn=true`、正文非空且不存在仍为 `in_progress` 的后继消息。短文本 final candidate 需要额外稳定观察，候选 `current_node` 或正文变化即重新计时；backend detail 暂时 429/5xx 时继续有界重试。只有下文明确列出的 `conversation_inaccessible` 与持续/交替 429 才允许在更长稳定窗口后使用有界 DOM fallback，普通错误禁止降级为 DOM 成功结果；已经观察到的 backend final candidate 即使随后遇到瞬时 429 也继续快速复核，不被降级到较慢的 DOM fallback。
...
- Daily Agent 不得把所有最终契约校验失败统一解释为“正文截断”。只有输出以当前目标日期的正确标题开头、已出现前置必需章节，但缺少契约中的最后必需章节时，才能在同一 conversation 请求续写。日期/标题错误、占位回复、完整但契约不匹配的正文和无可靠尾部标志的通用 Markdown 必须直接保留原始校验错误，不能循环发送“继续输出”。`io.stdoutTruncated` 只表示持久化的 stdout 事件预览达到上限；`result.response` 是独立的最终正文，不得用该 metadata 推断正文是否截断。
- ChatGPT Web 有时会把完整文档包在单个 `:::writing{...}` / `:::` 容器中。Daily Agent 只能在首行是 writing opener、末行存在独立 closing marker 时剥离这一层，再执行契约校验和保存；未闭合或嵌套位置不确定的容器保持原样并校验失败，防止把仍在生成的片段误当作完整报告。最终报告文件不得保留该 UI 容器。
