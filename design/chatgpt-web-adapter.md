...
- DOM fallback 的候选稳定性必须基于内容签名，而不是只看文本长度。签名至少覆盖 turn/message id、最终文本、自然批次数、图片数和附件数；同长度文本变化或新增批次都必须重置稳定窗口。`data-testid="stop-button"` 以及中英文 Stop/Cancel aria-label 是硬 busy 信号，必须继续等待；`Start dictation` / `Start Voice` / `开始听写` / `启动语音功能` 语音按钮配合 composer 可见、可输入且无待发送文本表示页面已空闲。页面空闲后不再额外等待 `stream_handoff` 固定宽限，只做短 DOM 签名稳定确认：图片和短文本 2 秒，中等文本 3 秒，超长文本 5 秒。发送按钮 selector 不得使用 `composer-submit-btn` 这类 stop button 也会携带的非语义 class。发送层在注入 composer 前、以及 send button 不可点击后，都必须重新检查 stop button；只要 stop button 可见，就清空可见 composer 中的待发送草稿，并在当前 send 流程内等待 stop button 消失后重新准备 composer。只有等待超时才返回不可整轮重试的 `conversation_busy`。微信/飞书等 IM 通道绑定 `chatgpt_web` runner 时，最终回写必须来自真正完成后的答案文本，而不是这类短 planning 段。
- `wait` operation 不能只依赖进程内的 `ConversationTab` 池。External runner 的 `send` 与 same-conversation `wait` 可能运行在不同短生命周期 worker 进程中；`send` 结束后真实浏览器 tab 仍打开，但内存池会随进程退出而消失。因此 DOM fallback 在本进程找不到 tab 时，必须通过当前共享 browser session 的 DevTools target 列表重新发现目标 `/c/{conversationId}`，连接 CDP、注册回池后再提取最终 assistant；只有浏览器里也找不到对应会话页时，才允许返回 `NotFound`。
...
- `required_dom_stable_for_keeps_completed_dom_responsive` 覆盖 stop button 消失后的 DOM ready 分支只需要短签名稳定窗口，避免 UI 已空闲后仍延迟 8-30 秒才回写 IM。
