...
- Project 深度研究可能先渲染一个没有 `data-message-id` 的 assistant 外壳，短暂恢复可用 composer 后再进入检索。此类 provisional shell 即使已有“我会先检索……”文本也不是 committed assistant output，必须继续等待；只有带 `data-message-author-role="assistant"` + `data-message-id` 的消息或已完成的生成图片可以进入终态稳定窗口。普通 `/c/...` 与 Project `/g/g-p-.../c/...` 必须使用同一规则。
...
- ChatGPT Web child 失败时，只要失败元数据中的题目、原始问题和 runner 与本次执行完全一致，且能恢复 conversation id，fan-out 重跑必须只对原 conversation 执行 `wait` 并重新校验五段正文；已有会话仍不完整时继续失败并明确拒绝创建重复 Pro 研究。Project 研究的成功/失败元数据和摘要链接都必须保持 `/g/g-p-.../c/<conversation-id>`，不能降级成普通 `/c/...` 链接。
