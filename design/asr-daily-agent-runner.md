...
Daily Agent Runner 是 ASR 定时任务的后处理阶段:ASR 音频转写成功、daily markdown 有增量后,自动为该 task 排队一个 Daily Agent 队列,按顺序把每个 enabled Agent 交给对应外部 Runner(ChatGPT Web / Codex / Claude Code / Trae X / 自定义)执行,输出 report 到 `.daily/agents/<agent_id>/output/<output_dir>/YYYY-MM-DD-report.md`,可选 IM 通道发送 summary 或 full report。配置 `report_sync_dir` 后,Agent report 按 Agent 子目录复制,原始 daily markdown 统一复制到目标根目录的 `原始文件/` 子目录。
...
- `report_sync_dir` 自动同步:Runner 成功生成 report 后复制到 Agent 子目录;ASR daily markdown 刷新后复制到目标根目录的 `原始文件/`;`last_report_sync` 与 `last_original_sync` 分别记录 copied/skipped/failed;外部目录超时不影响 ASR 或 report 成功状态。
...
- `report_sync_dir` 设置后 report 复制到 Agent 子目录,原始 daily markdown 复制到 `原始文件/`;iCloud 卡住时同步失败但 ASR 与 report 生成成功。
...
- `report_sync_dir`:本地目录复制,面向 iCloud、企业网盘等异步同步;Agent report 写入 `<root>/<agent_id>/`,ASR daily markdown 写入 `<root>/原始文件/`;失败只影响同步状态,不影响 ASR 或 report 生成成功。
...
ChatGPT Web plan 不包含 `unchanged`;文件型外部 runner 的 plan 不塞 daily markdown 全文(只传路径/摘要,减少 token)。
...
统一走 `ExternalCliRunRequest`（runner 为 Codex、Trae、Claude Code、ChatGPT Web 或自定义 external runner）：
...
- `ExternalCliRunRequest` 组装;ChatGPT Web 大输入剪贴板路径;Codex 等文件型外部 runner 直调。
...
- ChatGPT Web plan 不含 unchanged;文件型外部 runner 的 plan 不塞 daily markdown 全文。
...
- `report_sync_dir` 自动同步 + 手动同步;`原始文件/` 只接收合法的 `YYYY-MM-DD.md`;iCloud 卡住时同步失败但 ASR 与 report 成功;`last_report_sync` 和 `last_original_sync` 记录 copied/skipped/failed。
