...
- 必须实现：`crates/bifrost-admin/Cargo.toml` 只在 macOS aarch64 target dependency 中引入 `qwen3-asr` 和 `sherpa-onnx`。
...
- Cargo 依赖：
  - `qwen3-asr` 与 `sherpa-onnx` 放入 `[target.'cfg(all(target_os = "macos", target_arch = "aarch64"))'.dependencies]`。
...
| 平台 | ASR 入口 | Qwen3-ASR dependency | sherpa-onnx dependency | native diarization/voiceprint | capability |
| --- | --- | --- | --- | --- | --- |
| macOS aarch64 | 显示 | 编译 | 编译 | 编译 | enabled/hidden=false |
| macOS x86_64 | 隐藏 | 不编译 | 不编译 | stub | enabled=false/hidden=true |
| Linux x86_64 | 隐藏 | 不编译 | 不编译 | stub | enabled=false/hidden=true |
| Windows x86_64 | 隐藏 | 不编译 | 不编译 | stub | enabled=false/hidden=true |
...
- E2E 测试：
  - `e2e-tests/tests/test_asr_platform_gating.sh` 静态检查 Cargo target dependency、native cfg、CLI hide fence，并用 `cargo metadata --filter-platform` 验证 Linux 与 macOS x86_64 不解析 `qwen3-asr/sherpa-onnx`、macOS aarch64 会解析。
- 真实场景测试：
  - `human_tests/asr-platform-gating.md` 覆盖 macOS aarch64 支持平台入口显示、非支持平台入口隐藏、Cargo metadata 资源边界和 CLI help 隐藏。
