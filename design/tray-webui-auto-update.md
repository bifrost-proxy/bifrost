...
| 浏览器连接 desktop-owned core 后点击更新 | App 安装完成但没有 Tauri handoff，页面 reload 后假成功 | desktop-owned core 只接受带 Tauri 一次性 origin token 的 `channel=desktop`；普通浏览器请求返回 409 并提示从桌面 App 发起。WebView 的跨源预检必须在 `Access-Control-Allow-Headers` 中放行 `X-Bifrost-Desktop-Upgrade-Origin`，否则 token 虽已签发但 POST 不会到达 Admin，前端只会看到 `Network Error` |
| CLI 联动更新正在运行的旧 Desktop | 按 App 包前缀扫描时先命中 bundled core，把 `--bifrost-upgrade-shutdown` 误发给普通 CLI；按名称 `tell application "Bifrost"` 又可能退出同名的其它构建副本 | 内部 shutdown 参数只发送给精确的 `<App>/Contents/MacOS/bifrost-desktop`；兼容旧 App 的 macOS Quit 使用目标 App 完整路径。两条路径都必须继续等待目标 bundle 内所有进程退出，超时拒绝覆盖 |
...
- **单升级器所有权**:Admin 进程用互斥锁串行化 `check → claim → spawn`,实际 `self-update` 再持有 `<data_dir>/upgrade.lock` 跨进程排他锁。受控 CLI/App child 必须携带父进程为本次锁生成的随机 token 与 owner PID，并同时通过真实 OS parent PID、owner sidecar 和 live lock contention 三重校验后才能复用父事务；可见的 `source=cli-upgrade` 或调用者伪造的环境变量都不能绕过锁。CLI-owned orchestrator 启动 App companion 时，无论 caller-managed 即时安装还是 Windows desktop handoff，都显式传递该凭据；否则 child 会争用父锁并在 core 重启前终止统一升级。Windows pending desktop handoff 是锁释放后的延续 guard，竞争者不得覆盖它已有的 `Restarting` progress。Web UI 双击、两个浏览器、Tray 与 Web UI 同时发起时最多一个升级器能进入安装/重启阶段。
...
GET version-check 按 Web UI query 选择 CLI 或 App bundle 作为 primary，但 `has_update` 是 CLI 与已安装 App 的并集：主组件已到 target、伴随组件仍旧时也必须保留更新入口。当只有 companion 需要更新时，响应的 `current_version` 必须切换为该 companion 的实际当前版本，不能产生 `current_version == latest_version && has_update == true` 的自相矛盾状态。桌面弹窗同时使用通用的 Bifrost 更新文案，不把 companion CLI 更新误报成“新桌面 App”。POST upgrade 的版本门禁与实际 orchestrator 都由当前 core 的真实启动模式决定,避免过期或冲突 query 用错误组件做“无更新”判断:
...
- **App-owned**:`BIFROST_DESKTOP_CORE=1`。只有桌面 shell 先通过 Tauri command 签发短时、一次性 origin token，再发送带该 token 的 `channel=desktop` 请求，才能启动 desktop orchestrator；Admin CORS 预检必须显式允许 `X-Bifrost-Desktop-Upgrade-Origin`，确保 Tauri WebView 能把凭证送到后端并被原子消费。普通浏览器或仅伪造 query/header 的 REST 调用没有共享数据目录中的 token 文件，Admin 返回 409，避免没有 Tauri handoff 的调用者安装 App 后无法重启。App orchestrator 先调用独立 CLI 的 `upgrade -y`,同时注入 `BIFROST_DESKTOP_MANAGED_UPGRADE_SKIP_APP=1`、`BIFROST_DESKTOP_MANAGED_UPGRADE_SKIP_RESTART=1`、已验证父锁凭据与同一个目标版本,保证 CLI 子流程复用父锁、只更新精确目标 CLI,不会递归安装 App、停止 desktop core 或抢占重启。Windows deferred helper 从事务对象直接读取 pinned target，不从可缺失或陈旧的 progress 文件反推；父 App 收口时 helper 不发布 terminal progress，App 持续重试版本探针并统一决定成功或失败。desktop Admin 不覆盖 `--app-dir`，由 bundled core 的 `current_exe()` 解析正在运行的 App 副本。macOS 完成原子 App swap 后进入 `Restarting`；Windows 把 MSI/EXE 与“是否由 updater 下载”一起登记为 pending install，同样进入 `Restarting`。Web UI 只在 progress 同时满足 `source=desktop` 与 `phase=Restarting` 时调用 Tauri handoff；helper 等旧 App/core 退出后执行 Windows installer，仅清理 updater 自有包。新 App/core ready 后还要核对 App 编译版本与 pinned target，一致才写 `Completed`，不一致写 `Failed`。
- Windows deferred helper 的安装事务跨越旧 App、installer、新 App 三个进程阶段：旧 App/core 退出后先把安装目录快照到目标外并把 rollback metadata 原子写入 relaunch marker；installer 成功不等于提交，pending guard、快照与 updater 自有包继续保留。新 App 编译版本与新 core ready 后只负责持久化 `Completed`，不删除任何事务材料；仍在等待的 helper 读到该 durable terminal state 后才提交清理。任一不匹配、进度写失败、提前退出、120 秒验证超时或 core 启动失败都会保留快照并触发 helper 恢复完整旧目录、保持 `Failed` 并重新拉起旧 App。
- desktop channel 的 unified version-check 以已安装 App 为 primary。当实际 runtime 是 CLI-owned 时，当前正在服务的 core 就是本次统一检查的 CLI companion，必须直接使用当前编译版本，不得跳过 current executable 后把 PATH 或 `~/.cargo/bin` 里不生效的旧副本误当成伴随 CLI。只有 runtime 是 App-owned 时，才从与 CLI updater 相同的 PATH、`BIFROST_INSTALL_DIR`、用户安装目录和平台默认路径解析独立 CLI，并通过有界 `bifrost --version` 探针比较 companion。即使 App/core 已到 target，只要这个真实生效的 CLI companion 仍旧，WebUI 仍显示更新并进入同一个 pinned-target orchestrator；缺失、非零退出或超时才回退到 bundled core 版本。
...
- **共同结果**:两种入口都以同一个 pinned target 同时升级 CLI + 已安装 App；直接 `app upgrade --version` 也以 skip-App/skip-restart 行为把该 target 传给 CLI 引擎，不能递归编排或重新观察 `latest` 后发生版本分叉。所有顶层 CLI/App updater 共用 `upgrade.lock`，只有通过 token、真实父 PID、owner sidecar 与 live lock 校验的内部 companion 跳过重复加锁。重启所有权互斥：CLI-owned 由 CLI updater 重启 daemon，App-owned 由 Tauri handoff 重启 App/core；CLI-owned + WebView 混合模式中，CLI 重启 core 时必须保留 companion 已发布的 `Restarting/source=desktop`，避免抹掉 Tauri handoff 触发条件。
- **发起界面与运行时 owner 解耦**:Admin 不把 caller-controlled `channel=desktop` 当作 WebView 证明；桌面进程通过 Tauri command 在共享数据目录签发 UUID origin token，Web UI 放入专用 header，Admin 原子消费且校验 30 秒有效期后才向 CLI-owned orchestrator 传递私有 origin marker。不能仅凭 `RuntimeStartMode::Desktop` 推断 WebView 正在等待 handoff。已安装桌面 shell 正在运行且请求携带真实、未消费的 WebView token 时，CLI companion 才发布 `source=desktop/Restarting` 交给 Tauri；CLI-owned core 仍由 CLI updater 重启，Tauri marker 的 `old_core_pid` 不包含未托管的 core child，但会记录观察到的外部 PID 与 pinned target。Desktop shutdown 对该 marker 保留外部 core，helper 只等待旧 App 退出，不等待外部 PID/端口释放。新 App 以 pinned target version 为成功权威，允许复用 CLI updater 已经完成重启的同 PID target core；只有 legacy marker 缺 target 时才继续要求 PID 变化。等待 target core 超时后，端口空闲则启动 bundled core；端口由同 data dir `runtime.json` 精确匹配的旧版本 Bifrost core 占用时先安全停止再接管；无关端口占用仍 fail-closed。旧 Desktop 已留下同类 Failed progress 时，新版本首次启动零等待重试并在成功后清 marker；健康但错误版本的 core 不能绕过该恢复门禁。终端或普通浏览器发起时不允许留下等待 WebView 的 pending install：macOS/Windows companion 先把 single-instance 内部 shutdown 参数精确发送给目标桌面壳 executable，不能把它误发给同一 bundle 内的普通 CLI core；不支持该参数的旧 App 回退到平台退出，macOS 必须用完整 bundle 路径而不是可能命中其它构建副本的应用名。Git 历史确认 `v0.0.17-beta` 起的 144 个 Desktop 版本都处理系统 Quit 并清理 managed core，`v0.0.157` 起额外支持内部参数。只有确认目标 App bundle 内全部 executable 已释放后才执行 caller-managed 安装并重新拉起；退出超时则拒绝覆盖，不能冒险替换锁定文件。多 App 副本并存时 companion 优先选择实际正在运行的候选。
...
pub fn write_progress(&Path, &UpgradeProgress); // 唯一同目录 tmp + sync + 原子 replace；Windows 瞬态共享冲突退避重试
...
- **旧 Desktop 已留下 CLI-owned handoff 失败状态**:新 App 读取 fresh relaunch marker 和同类 `Failed` progress 后不得再重复等待 30 秒；同 target marker 立即重试，旧 marker 没有 target 时也按兼容恢复处理。若目标版本 core 已健康则完成并清 marker；端口空闲则启动 bundled core；同 data dir 的旧版本 core 可安全停止后接管；无关 listener 保持 fail-closed。Desktop shutdown 和 relaunch helper 都不得再次停止或等待 CLI updater 已经拉起的 external core。
