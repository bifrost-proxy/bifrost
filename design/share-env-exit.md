...
- 注入页面的 Bifrost badge JSON 增加 `share_env` 字段；当 `share_env.active=true` 时，B 胶囊立即进入 Share 视觉状态：边框呼吸光晕、右上角红点，hover panel 中展示短文案 `Share mode active` 和 `Exit` 按钮。
...
- `bifrost-proxy`：`test_badge_share_env_badge_and_exit_button_present` 验证注入片段包含 Share 红点、呼吸光晕、初始化 Share 状态、`Share mode active` 行、Exit 按钮和退出 API 的 `no-cors` 兜底。
...
- `e2e-tests/tests/test_badge_injection_e2e.sh`
  ...
  - 断言 Share 状态 API active，注入页面包含 Share 红点、呼吸光晕、`Share mode active` 和退出 API。
...
- 通过真实代理访问分享链接，确认页面胶囊出现呼吸光晕和右上角红点，hover panel 显示 `Share mode active` 和 `Exit`。
- 点击或调用 Exit，确认规则状态恢复。
