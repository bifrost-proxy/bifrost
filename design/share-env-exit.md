...
- 注入页面的 Bifrost badge JSON 增加 `share_env` 字段；当 `share_env.active=true` 时，胶囊右下角显示 `Share` badge，hover panel 中展示 `{requested_name} share env active` 和 `Exit` 按钮。
- Exit 按钮调用本地 admin API，成功后刷新当前页面，让后续请求使用恢复后的规则状态。
...
- `bifrost-proxy`：`test_badge_share_env_badge_and_exit_button_present` 验证注入片段包含 Share badge、`share env active` 行、Exit 按钮和退出 API。
...
- `e2e-tests/tests/test_badge_injection_e2e.sh`
  ...
  - 断言 Share 状态 API active，注入页面包含 Share badge 和退出 API。
...
- 通过真实代理访问分享链接，确认页面胶囊右下角显示 Share badge，hover panel 显示 `share-source share env active` 和 `Exit`。
- 点击或调用 Exit，确认规则状态恢复。
