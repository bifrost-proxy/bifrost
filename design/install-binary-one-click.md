...
- 代码 review：检查 upgrade 镜像探测不会破坏用户指定 `BIFROST_GITHUB_MIRROR`，被选中源失败后仍能回退旧下载路径；检查 PowerShell env 变量、latest、archive、checksum 下载路径与 Bash installer 保持一致；检查 upgrade restart 不会在端口仍被占用时启动新 daemon，且失败时会恢复系统代理。
