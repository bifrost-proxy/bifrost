...
export default function CertificateTab({
  ...
  const handleCreateTrustProbe = useCallback(async () => {
    ...
    try {
      ...
      message.success("Trust probe QR code is ready.");
    } catch (error) {
      ...
      <Col xs={24} md={18} lg={19} xl={20}>
        ...
        <Card
          ...
          title={
            <Space>
              <MobileOutlined />
              <span>Universal Trust Probe</span>
            </Space>
          }
          size="small"
        >
          <Space direction="vertical" style={{ width: "100%" }} size="middle">
            <Alert
              type="info"
              showIcon
              message="Verify trust from the device itself"
              description="Scan the HTTP landing page with any phone, tablet, or browser. The page first checks the probe port, then attempts a real HTTPS request signed by the current Bifrost CA."
            />
