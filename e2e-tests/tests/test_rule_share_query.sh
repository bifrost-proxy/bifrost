...
grep -F 'rsq-e2e [enabled]' /tmp/bifrost-rule-share-list-1.txt
...
COUNT="$(grep -Ec '^  rsq-e2e( [0-9]+)? \[' /tmp/bifrost-rule-share-list-2.txt || true)"
[[ "$COUNT" == "1" ]]
...
grep -F 'rsq-e2e [disabled]' /tmp/bifrost-rule-share-list-3.txt
grep -F 'rsq-e2e 2 [enabled]' /tmp/bifrost-rule-share-list-3.txt
...
COUNT_AFTER_SUFFIX="$(grep -Ec '^  rsq-e2e( [0-9]+)? \[' /tmp/bifrost-rule-share-list-4.txt || true)"
[[ "$COUNT_AFTER_SUFFIX" == "2" ]]
grep -F 'rsq-e2e 2 [enabled]' /tmp/bifrost-rule-share-list-4.txt

API_RESP="$(
  curl -fsS -X POST "http://127.0.0.1:${PROXY_PORT}/_bifrost/api/rules/share-link" \
    -H 'Content-Type: application/json' \
    --data "{\"name\":\"rsq-e2e 2\",\"target_url\":\"http://127.0.0.1:${TARGET_PORT}/from-api\"}"
)"
python3 - "$API_RESP" <<'PY'
import json
import sys
...
assert resp["rule_name"] == "rsq-e2e 2"
