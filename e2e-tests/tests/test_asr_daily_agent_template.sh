...
require_text "这份指南会作为当前 Daily Agent 工作目录中的 \`AGENTS.md\` 写入"
require_text "下面所有规则都是运行指令，不是注释、示例或可忽略说明"
require_text "不能省略会影响输出契约的核心模块"
...
require_text "方案草案"
require_text "未能联网搜索"
require_text "不是装饰性标题"
require_text "日报、复盘、待办追踪和长期记忆索引"
