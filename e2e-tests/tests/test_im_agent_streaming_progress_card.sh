...
if [[ -n "${FEISHU_APP_ID:-}" && -n "${FEISHU_APP_SECRET:-}" && -n "${FEISHU_OWNER_OPEN_ID:-}" ]]; then
  ...
  echo "[im-agent-streaming-progress-card] Local shell E2E verified JSON 2.0 streaming card structure, fixed CardKit element ids, coalesced assistant stream prose, one final answer, retained thinking/tool/status progress, guide/queue footer state, mock Feishu freeze-and-rollover behavior, finished-card retention, and turn-end inbound drain behavior."
fi
