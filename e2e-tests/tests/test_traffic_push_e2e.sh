...
test_ws_connection_via_http_proxy_to_admin_rejected() {
    log_info "Testing admin push WebSocket spoofing via HTTP proxy is rejected..."
    ...
    if output=$(node "$SCRIPT_DIR/../test_utils/ws_via_http_proxy.js" \
        ...
        log_fail "Proxy WebSocket connection unexpectedly succeeded: $output"
        return 1
    fi

    log_debug "Proxy WS output: $output"
    assert_contains "$output" "Unexpected server response: 403" "Proxy WebSocket spoofing should be rejected with 403" || return 1
    return 0
}
...
main() {
    ...
    if [[ "$has_websocat" == "true" ]]; then
        ...
        run_test "WebSocket Admin Push Via HTTP Proxy Rejected" test_ws_connection_via_http_proxy_to_admin_rejected
