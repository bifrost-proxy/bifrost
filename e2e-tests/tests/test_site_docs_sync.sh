...
ZH_SCRIPTING_DIST="$SITE_DIR/dist/reference/scripting.html"
EN_SCRIPTING_DIST="$SITE_DIR/dist/en/reference/scripting.html"
...
test -f "$EN_RULE_DIST"
test -f "$ZH_SCRIPTING_DIST"
test -f "$EN_SCRIPTING_DIST"
grep -q 'resStreamScript' "$ZH_SCRIPTING_DIST"
grep -q 'Streaming Response Script' "$EN_SCRIPTING_DIST"
grep -q 'Transform' "$ZH_SCRIPTING_DIST"
grep -q 'Mock' "$ZH_SCRIPTING_DIST"
grep -q 'Transform' "$EN_SCRIPTING_DIST"
grep -q 'Mock' "$EN_SCRIPTING_DIST"
grep -q '立即把返回值发送给客户端' "$ZH_SCRIPTING_DIST"
grep -q 'sends the returned output immediately' "$EN_SCRIPTING_DIST"
grep -q 'timeout_ms' "$ZH_SCRIPTING_DIST"
grep -q 'timeout_ms' "$EN_SCRIPTING_DIST"
grep -q 'href="/bifrost/reference/rules/scripts"' "$ZH_SCRIPTING_DIST"
grep -q 'href="/bifrost/en/reference/rules/scripts"' "$EN_SCRIPTING_DIST"
