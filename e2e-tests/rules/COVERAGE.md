...
- `tests/test_desktop_service_ownership_lifecycle.sh`：在 macOS 临时 data-dir 和动态端口中暂停 Desktop-owned Service，并保持暂停直到 watchdog 完成恢复门槛与最终确认，断言受控终止无响应 PID 并在恢复预算内拉起健康新 PID；随后真实终止子进程，断言 child-exit 路径仍通过 restart-preserving marker cleanup 恢复，同时保留 Desktop/CLI ownership 边界。
