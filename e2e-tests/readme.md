...
| `test_feishu_progress_terminal_notification.sh` | 1 | 飞书任务过程卡成功/失败终态折叠、独立终态卡引用链、本地文件自动上传发送，以及出站 100 MiB 上限/上传失败的非阻塞提示 |
| `test_im_outbound_send_e2e.sh` | 1 | IM provider 能力发现、provider/飞书 bot ID/名称选择、owner/target/直达群聊、Markdown/图片/文件/原生卡片有序发送、超过旧 32 MiB worker 门槛的大文件 raw binary upload、危险附件文件名拒绝与严格 CLI 参数校验 |
