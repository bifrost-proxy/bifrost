...
| `test_im_gateway_live_model_switch.sh` | 1 | 运行中的 IM `/model` 经隔离 worker、主进程 broker 与 external worker 下发 Codex app-server `thread/settings/update`，并验证 set/clear、持久化与后续轮次语义 |
