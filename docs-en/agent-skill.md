...
Traffic skills should use newest request time first, sequence descending for ties, and literal substring `--path` filters. Search `--limit` defaults to 50 and explicit `--max-results` overrides it. Use `--format json` for filter-only queries; see the [CLI reference](./cli.md) for JSONPath, header equality, and time windows. Relay `remote traffic` has no batch get or search include wrapper; unsupported capabilities must not silently expand shell authorization or switch connection modes.
