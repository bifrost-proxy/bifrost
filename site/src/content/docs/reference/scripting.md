...
> 规则侧如何引用脚本（`reqScript://` / `resScript://` / `resStreamScript://` / `decode://` / `bp://`）请参考[脚本规则参考](./rules/scripts)。通过 CLI 或 AI 生成、分享规则时，默认推荐把脚本写成规则文件内的 inline block；Scripts 页面主要用于交互式测试、试跑和人工维护命名脚本。

---

## 1. 脚本类型与执行时机

Bifrost 当前实现支持五类脚本能力：

1. **Request Script**：转发到上游前执行，可修改请求（方法/头/body）。
2. **Response Script**：收到上游响应后执行，可修改响应（状态码/头/body）。
3. **Streaming Response Script**：通过 `resStreamScript://` 对 SSE 响应逐事件转换，或由脚本主动生成 Mock SSE；每次输出都会立即发送，不会先收集完整响应。
4. **Decode Script**：用于对 body 做“解码/脱敏/格式化”等处理（常用于展示与落库前的处理）。
5. **Parser Script**：用于 `bp://...` + `decode://bp` 的二进制协议解析。它不会改写真实客户端/上游流量，只影响 Traffic 落库、详情展示与搜索。

管理端 Scripts 页面会展示 request / response / decode / parser 四个存储分类，流式响应脚本与普通响应脚本都存放在 response 分类。列表、保存、删除、重命名、测试走 Admin API `/_bifrost/api/scripts/*`；包含 `stream.mode` 的响应脚本需要通过 `resStreamScript://` 规则和真实 SSE 请求验证，不能用普通 Response Script 的完整 body 试跑代替。
...
- 存储根目录：`{data_dir}/scripts`
  - 默认 `data_dir=~/.bifrost`（可用 `BIFROST_DATA_DIR` 覆盖：`crates/bifrost-storage/src/data_dir.rs:37`）
...
对应实现：`crates/bifrost-script/src/engine.rs:1379`（`validate_script_name`）。
...
说明：脚本运行环境为 **QuickJS**，同步执行，不支持 `async/await`（见 `crates/bifrost-script/src/sandbox.rs:102`）。
...
- `ctx`：上下文（`requestId/scriptName/scriptType/values/matchedRules/phase`），注入逻辑见 `crates/bifrost-script/src/sandbox.rs:624`。
- `log` / `console`：日志对象（`log.debug/info/warn/error`），注入逻辑见 `crates/bifrost-script/src/sandbox.rs:523`。
- `file`：文件 API（受沙箱目录与白名单限制），注入逻辑见 `crates/bifrost-script/src/sandbox.rs:1105`。
- `net`：网络 API（可开关/限超时/限包体大小），注入逻辑见 `crates/bifrost-script/src/sandbox.rs:1274`。
...
请求对象注入：`crates/bifrost-script/src/sandbox.rs:688`。
...
响应对象注入：`crates/bifrost-script/src/sandbox.rs:740`。

### 3.4 Streaming Response Script

流式响应脚本使用 `resStreamScript://{script_name}` 或规则文件内的 inline block。每条 HTTP 响应流拥有独立且持久的 QuickJS 上下文，因此同一条流中的顶层变量和闭包状态会跨 event 保留，不同请求之间互相隔离。

它支持两种真正的流式模式：

- **Transform**：上游必须返回 `Content-Type: text/event-stream`。Bifrost 拼出一条完整 SSE event 后调用一次 `stream.onEvent(event)`，并立即把返回值发送给客户端，不等待上游 EOF 或 `[DONE]`。
- **Mock**：脚本通过 `stream.next()` 主动逐步生成 SSE。每次返回的输出先发送，再根据可选的 `delayMs` 等待下一步，不会预生成整段响应。
...
`stream.next()`、`stream.onEvent(event)` 与 `stream.onEnd()` 可以返回原始 SSE 字符串、SSE event 对象、数组、带 `output` / `outputs` 的 step 对象，或用 `null` / `undefined` 表示本次不输出。单个完整 event 的安全上限为 16 MiB；超限或流开始后的回调错误会以明确的 SSE error 终止，不会静默截断。

流式回调仍同步执行，不支持 `async/await`。沙箱 `timeout_ms` 在每次 JavaScript 回调开始时重新计时，只限制该次回调的 CPU 执行时间；等待上游下一条 event 的时间不累计。当前一条匹配结果只允许一个 `resStreamScript`，并且不能与收集完整 body 的 `resScript` 组合。

完整的 inline block 写法、event 字段、返回值和响应头约束见[脚本规则参考](./rules/scripts)。
...
decode 注入与截断逻辑：`crates/bifrost-script/src/sandbox.rs:812`。

decode 输出约定：脚本需要输出 `{ code, data, msg }`（用 `ctx.output = {...}` 或全局 `output = {...}` 赋值），解析逻辑：`crates/bifrost-script/src/sandbox.rs:1024`（`parse_decode_output_from_js`）。

> 注意：当前 QuickJS 构建不会把脚本体包进函数，因此**顶层 `return` 会直接报错**（`Exception generated by QuickJS: return not in a function`）。请用 `ctx.output = {...}`（或 `globalThis.output = {...}`）输出结果，不要在脚本顶层写 `return`。
...
Parser 复用 decode 输出约定，成功时输出 `{ code: "0", data, msg: "" }`（同样用 `ctx.output = {...}` 赋值，不要写顶层 `return`——parser 与 decode 走同一套不包函数的 eval，顶层 `return` 会报 `return not in a function`）。`data` 会替换落库后的 body 文本用于详情展示与 `bifrost search --req-body/--res-body`，但不会改写真实转发流量。
...
if (net.enabled) {
  // net.fetch 需要两个参数：URL 与 options JSON 字符串（第二个参数必须存在，无选项时传 "{}" 或 null）。
...
- 读取：`crates/bifrost-admin/src/handlers/config.rs:218`（`get_sandbox_config`，路由分发在 :162）
- 更新并持久化：`crates/bifrost-admin/src/handlers/config.rs:263`（`update_sandbox_config`，路由分发在 :164）

配置结构定义：`crates/bifrost-storage/src/unified_config.rs:58`（`SandboxConfig`；数值限制默认值见 `SandboxLimitsConfig`，约 `unified_config.rs:105-122`）。
...
- `sandbox.limits.max_memory_bytes = 33554432`（32MB），作为 QuickJS 运行时内存上限（`crates/bifrost-script/src/sandbox.rs:103`，`set_memory_limit`）
