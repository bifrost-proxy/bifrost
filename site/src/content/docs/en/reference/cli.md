...
When an IM channel comes online, Bifrost sends the online notification followed by help for the bound external Runner. All runners receive channel commands such as `/help`, `/status`, `/cwd`, `/runner`, `/q`, `/rq`, and `/stop`. Codex, Traex, and Claude Code advertise `/models`, `/model`, `/efforts`, and `/effort` only when the adapter supports them.
