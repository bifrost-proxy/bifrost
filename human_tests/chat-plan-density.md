...
- Plan step 超过 5 条时必须在详情浮层的 step 列表内部滚动。
...
### TC-CPD-01: Plan 胶囊与 hover 详情紧凑展示
...
3. 悬浮 Plan 胶囊打开详情浮层，检查每条 step 左侧状态展示。
...
- 不再展示 `Completed`、`In Progress`、`Pending` 文字 tag；详情中允许以低对比度文字补充状态。
...
2. 悬浮 Plan 胶囊，检查 `agent-chat-plan-list` 的可视高度和滚动状态。
3. 滚动 plan list 到底部。
...
2. 切换到暗色主题。
3. 重复检查 Plan 胶囊、hover 详情浮层、step list、输入框和发送按钮位置。

**预期结果**

- 亮色和暗色主题下 Plan 胶囊、详情浮层、状态图标、step 文字和输入框 hint 均清晰可读。
- 发送按钮仍位于输入框右下角，不遮挡输入文本或 hint。
- 鼠标离开胶囊/详情浮层后不显示 step list，重新 hover 后仍维持 5 条高度上限。
