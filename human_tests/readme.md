...
| [asr-external-device-import.md](./asr-external-device-import.md) | ASR 外接设备自动导入 | 11 | ASR 任务绑定多个外接设备名称、V1 只承诺 macOS 且 macOS 必须完整覆盖 Disk Arbitration 连接事件监听、真实 disk image/外接设备验证，以设备名称作为 `audio_dir` 下根目录、保持设备内目录结构和文件名、差异导入去重、导入复制流同步计算 SHA-256 并在 ASR 处理前按内容 hash 跳过已完成且产物存在的重复转写、文件大小/稳定性检查、打开 ASR 任务配置页时发现当前已连接设备并用单个列表弹窗确认，确认后加入绑定并立即导入，已绑定且已连接设备可在任务列表点击 `Import External` 手动导入，不做后台定时扫描，ASR 定时任务创建后可编辑名称、数据源目录、启动时间、定时周期和所有配置且切换数据源不影响已转写数据，删除任务必须在危险弹窗中输入完整任务名称确认，同名设备冲突、设备断开、权限不足和目标空间不足等容错状态，并保留 Linux/Windows provider 后续扩展架构 |
| [asr-realtime-voice-input.md](./asr-realtime-voice-input.md) | ASR 实时语音输入与本地 Voice Input Runtime | 18 | Web 麦克风实时输入走 `/api/voice/listen-ws` 真流式 PCM16、WebSocket PCM 协议与取消、CLI 麦克风监听、CLI 来源枚举、系统音频监听状态、应用音频来源选择、自定义词汇纠错、后置本地改写、隐私边界、长会话性能稳定性、离线 ASR server 与实时 stateful 策略区分、Rust qwen3-asr 0.6B stateful streaming provider 真实流式实验、实时 0.6B 启动时初始化且离线默认保持 1.7B、Web 采样率重采样、独立 worker 进程资源隔离、partial/stable/final 提交边界、静音提交、worker IPC timeout、最长 utterance、WebSocket idle unload、持续静音不反复拉起 worker、真正 InputMethodKit 输入法安装与 IMK marked/commit、用户编辑反馈学习、热键取消/撤销语义、麦克风设备选择 |
