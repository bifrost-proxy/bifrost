...
| [remote-invoke-file.md](./remote-invoke-file.md) | Remote Invoke File API (Phase 1/2/3) | 22 | file.read/list/stat/glob/search/hash/write/edit/mkdir/mv/rm/apply-patch 的正向用例、FileAccessPolicy 的 roots/denies/symlink-escape/scope/二进制/target 拒绝用例、shell scope 不自动授予 file API、并发与 grant 过期稳定性、审计日志留痕、CLI --help / --output json UX 验收，以及与既有 remote command/shell 能力的回归 |
