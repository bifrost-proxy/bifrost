...
| [install-binary-one-click.md](./install-binary-one-click.md) | 二进制安装脚本一键体验 | 9 | `install-binary.sh` 与 `install-binary.ps1` 默认自适应选择更快 GitHub/mirror 下载源，post-install 自动安装并信任 CA、安装所有 Bifrost skills、后台启动服务，覆盖 Bash/PowerShell 下载源回退、临时目录真实安装、命令顺序、全局 opt-out、分步 opt-out 和 help 文案 |
