...
| [grant-file-access.md](./grant-file-access.md) | Grant File Access 正交权限模型 | 13 | file_access 独立于 grant_scope 的正交权限模型：WebUI 预设策略模式（Full Access/Shell Only/File Only/Query Only/Custom）、PairingRequestModal 审批传递 file_access、Grant Editor 预设选择器、CLI --file-access 参数、API approve/update file_access、file_access Tag 显示、权限检查（none 阻止文件操作、read 阻止写操作）、旧 remote_file_read/write scope 不再支持 |
