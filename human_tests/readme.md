...
| [daily-agent-template.md](./daily-agent-template.md) | Daily Agent 默认模板 | 3 | 默认 AGENTS.md 模板要求知识沉淀在同一份 report 内分模块输出，不默认创建、建议创建或引用 `knowledge/*` 额外目录；覆盖长期想法、方向决策、跨天待办、人物协作、学习资料、生活状态和术语误识别模块；灵感爆发时刻必须补充资料搜索、可行性分析、方案草案和下一步验证 |
