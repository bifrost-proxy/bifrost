...
| [remote-shell-exec.md](./remote-shell-exec.md) | Remote Shell Exec | 21 | `bifrost remote command exec` 主链路回归：caller 不再传 `policy_id`、target 基于 grant binding 与本地 Shell Access 自动选择唯一策略、query/shell scope 隔离、策略未命中与歧义匹配拒绝、policy version 失效、`Full Access` / `Default Sandbox` 真实语义、grant 的 WebUI / CLI 编辑、target 本地 grant policy overlay 持久化、relay 仅保留最小 `grant_scope` 不存储具体策略绑定，以及 reconnect 覆盖旧 grant / disconnect 清理残留 reusable grants 的验证；新增 CLI 对裸 argv 输入的显式拒绝回归 |
