...
| [asr-platform-gating.md](./asr-platform-gating.md) | ASR Platform Gating | 5 | macOS aarch64 支持矩阵、capability API、非支持平台隐藏 WebUI/CLI ASR 入口、Cargo target dependency 不解析 qwen3-asr/sherpa-onnx、native diarization/voiceprint cfg 边界 |
...
| [remote-invoke-sshkey.md](./remote-invoke-sshkey.md) | Remote Invoke SSH Key CLI 与 Caller Identity | 2 | 本机 CLI 快速生成/导出/revoke Remote Invoke SSH key，且同一 SSH key 在多个 caller 沙箱中连接同一 target 时生成不同随机 caller ID，grant 以 caller ID 隔离且 SSH key fingerprint 仅作为密钥属性保留 |
...
| [api-sync.md](./api-sync.md) | Sync API | 32 | 同步状态/配置/登录/登出/运行/Session，Env/Room/User 代理转发端点，CI/沙箱 token-only 默认 Provider 与 token+URL 直登 |
...
| [proxy-rules-advanced.md](./proxy-rules-advanced.md) | 规则协议全量测试 | 73 | 40+ 规则操作协议：请求/响应修改、内容注入、控制、路由、脚本、高级特性（Values 引用、模板字符串、正则捕获），含 html/js/css 内容注入协议矩阵、htmlPrepend 插入 `<html>` 后、htmlAppend 插入 `</html>` 前，以及 HTTPS 转发到 HTTP 上游、gzip HTML 响应编码一致性、gzip JSON reqMerge/resMerge 合并回归、HTTPS 解包 gzip JSON reqMerge/resMerge 合并回归、脚本与 mock 路径压缩 Body 修改回归、HTTPS 解包链路 `resScript` 回归、mock 生成资源、通配域名根路径 htmlAppend 匹配、culture.shtml HTTPS MITM 背景图白屏、上游 HTTP/2 body 断流 fallback、无规则命中时已知长度响应头透明转发真实回归、reqHeaders Markdown value 中 `#` 注释行回归 |
...
| [ci-shell-e2e-sharding.md](./ci-shell-e2e-sharding.md) | CI Shell E2E 测试分片优化 | 31 | --shard N/M 参数解析、环境变量透传、分片覆盖完整性、无分片向后兼容、local-ci.sh 分片支持、单分片在 CI 预算内完成、CI skip 列表、格式校验、系统代理用例 CI 禁跑且本地保留、隐藏日志 artifact 上传与失败摘要诊断、CLI offline help alternation 回归、CLI offline Broken pipe 回归、失败日志 dump pipefail guard 回归、unsafe_ssl 自带 HTTPS mock fixture 回归、并行 shell 调度器与顶层 final status 全 PASS 后返回 0 回归、SSE replay timeout 边界回归、macOS CI post-timeout 连接噪声回归、unsafe_ssl 管理端端口碰撞回归、long-term memory frontend build 竞争回归、Agent/IM human-api 并行端口隔离回归、Agent history/direct-path 与 Feishu card 预构建 release binary 复用回归、remote relay fallback 预构建 binary 复用回归、Linux/macOS shell E2E timeout 预算回归、main push CI concurrency 取消旧 run 回归、Linux/macOS shell shard 内部并发预算回归、shell E2E 默认 Cargo 解析回归、site docs sync 缺失 site 依赖自举回归、Linux shell E2E 非交互 CA gate 回归、Cargo-heavy shell 用例串行调度避免 artifact lock 超时回归、HTTP3/Replay 外部 httpbin 依赖本地 mock 化回归 |
| [e2e-port-allocation.md](./e2e-port-allocation.md) | E2E Port Allocation | 1 | Rust E2E 并发测试中 request modification proxy 启动遇到端口 bind 竞态时重新选择端口并重试，避免 `portpicker` 探测后实际 bind 前被抢占造成 workspace test 偶发失败 |
| [ci-e2e-runner.md](./ci-e2e-runner.md) | CI E2E Runner 并发稳定性 | 4 | `bifrost-e2e` 并发 runner 对进程级副作用 standalone 用例提供 serial-only 隔离，覆盖 IM Gateway 长期记忆、session 恢复、ChatGPT Web conversation 恢复和 Remote shell stdout streaming CI 阈值回归 |
| [ci-macos-cli-e2e-split.md](./ci-macos-cli-e2e-split.md) | CI macOS CLI/E2E 构建拆分 | 6 | macOS rules/shell E2E 仅等待 aarch64 CLI 构建，desktop bundle 与 x86_64 CLI 构建不阻塞 E2E，并确保 Tauri desktop bundle 使用真实 Cargo/Rustc 工具链；macOS desktop bundle 的 Rust toolchain 安装对 static.rust-lang.org DNS 抖动进行有限重试 |
...
| [install-binary-one-click.md](./install-binary-one-click.md) | 二进制安装脚本一键体验 | 10 | `install-binary.sh` 与 `install-binary.ps1` 默认自适应选择更快 GitHub/mirror 下载源，post-install 自动安装并信任 CA、安装所有 Bifrost skills、后台启动服务，覆盖 Bash/PowerShell 下载源回退、临时目录真实安装、命令顺序、全局 opt-out、分步 opt-out、help 文案和 CI Cargo 依赖下载 HTTP/2 抖动回归 |
...
| [im-guide-queue-mode.md](./im-guide-queue-mode.md) | IM 引导模式和排队模式 | 16 | SessionQueueManager 单元测试（14项，含多 guide 累积、pending status、turn-end guide drain 与 guide 优先于 queue 回归）、guide_channel 字段集成、服务启动、API 验证、handle_busy_message runner-aware 路由（内置 Bifrost Agent 默认引导、/q 排队、/rq 删除、ChatGPT Web/Codex/自定义 Runner 默认排队）、Codex Runner `exec resume` 接续能力边界、tokio::select! 交错处理、并发事件路由、mid-turn 注入、`/agent/chat` 注入式 guide/queue 黑盒真实链路（多 guide `/status` 明细与合并消费、turn-end guide、FIFO drain、guide 优先、空白忽略）、全量测试 |
...
| [update-plan.md](./update-plan.md) | Update Plan 工具 | 3 | 真实 Bifrost + Admin API + mock model server 黑盒验证 update_plan 工具注册、runtime 强制收口未完成计划、同一 turn 最终快照不继承早先 completed 步骤、同一 session 下一轮新任务计划不继承旧 completed 步骤、`plan: []` 清空当前快照、`plan_steps` 最终返回与 helper 回归测试 |
