...
| [ci-shell-e2e-sharding.md](./ci-shell-e2e-sharding.md) | CI Shell E2E 测试分片优化 | 30 | --shard N/M 参数解析、环境变量透传、分片覆盖完整性、无分片向后兼容、local-ci.sh 分片支持、单分片在 CI 预算内完成、CI skip 列表、格式校验、系统代理用例 CI 禁跑且本地保留、隐藏日志 artifact 上传与失败摘要诊断、CLI offline help alternation 回归、CLI offline Broken pipe 回归、失败日志 dump pipefail guard 回归、unsafe_ssl 自带 HTTPS mock fixture 回归、并行 shell 调度器与顶层 final status 全 PASS 后返回 0 回归、SSE replay timeout 边界回归、macOS CI post-timeout 连接噪声回归、unsafe_ssl 管理端端口碰撞回归、long-term memory frontend build 竞争回归、Agent/IM human-api 并行端口隔离回归、Agent history/direct-path 预构建 release binary 复用回归、remote relay fallback 预构建 binary 复用回归、Linux/macOS shell E2E timeout 预算回归、main push CI concurrency 取消旧 run 回归、Linux/macOS shell shard 内部并发预算回归、shell E2E 默认 Cargo 解析回归、site docs sync 缺失 site 依赖自举回归、Linux shell E2E 非交互 CA gate 回归、Cargo-heavy shell 用例串行调度避免 artifact lock 超时回归、HTTP3/Replay 外部 httpbin 依赖本地 mock 化回归 |
...
| [agent-token-usage.md](./agent-token-usage.md) | Agent Token Usage 统计口径 | 4 | 区分累计 `total_tokens_used` 与当前 context `last_response_tokens`，Chat Completions `prompt_tokens` / Responses `input_tokens` 作为 context 快照，JSONL `context_tokens` 持久化与旧事件兼容，单元和真实服务 E2E 验收入口 |
...
| [im-gateway-agent.md](./im-gateway-agent.md) | IM Gateway Agent 对话能力 | 129 | AI 一级页整合 Agent 与 IM Gateway 子导航、Agent Chat 真实流式 API、Slash Runner Call、JSONL 历史安全恢复与续聊、active 会话刷新恢复、线程列表滚动与唯一选中、Threads 默认 20 条分批加载、Load more 每次追加 20 条、Threads 虚拟滚动、Threads 去重与 Runner/渠道标签分离、线程右键菜单原位确认删除、空白入口无 demo 消息且有线程时默认选中第一条、三类 Runner 五轮多轮会话不漂移、顶部来源/Runner/状态标签、Status 弹窗回填 Workspace/Status/Context 且不展示 Tools 模块、已初始化会话 Workspace 锁定、New Chat 弹窗选择 workspace/runner 且空白新会话不重复生成 session id、Plan 输入框上方折叠展示、刷新/关闭页面不中断后台 Agent Loop、sessions/all 统一 title fallback 与去重、Codex Runner 完成后会话不消失、服务端稳定 title、历史阅读不锁底、消息时间戳、assistant 气泡满宽、运行中输入 Guide/Queue/Stop、对话内容轨道 750px 居中、运行中不显示消息区全局 Loading 图标、Composer 键盘交互与旧会话深链兼容、Agent 配置 API、分层 prompt、Sessions/History/WebUI、飞书消息触发 Agent、多轮上下文、/clear、/status、/stop、MCP/Skills/AGENTS.md、Provider 级 Agent 配置、动态工作目录、Agent 默认发送通道、send_msg 默认消息通道、send_msg 统一工具与 schedule 绑定消息通道、send_msg 默认通道真实链路、真实用户默认 IM 通道 send_msg 模型兼容链路、ChatGPT Web 长回复 artifact 不截断且 Weixin 失败后拆分补发、Agent 模型 reasoning 参数可在 WebUI 关闭、Provider Connection 与 Memories 默认值 placeholder、Provider 卡片详情布局与操作按钮去重、Runtime Settings 恢复默认值、retry 与 orphan tool 防回归、飞书卡片折叠面板与流式进度卡片（Agent loop CardKit streaming progress、Token/Context K/M/B 格式化、标题跟随 set_title、可选 plan/tool/thinking 模块、plan 标题显示当前任务、thinking 标题显示一行摘要、最终输出置底、guide/queue 同卡刷新折叠状态区并在标题给出轻量可见反馈）、/status runner 元信息与压缩次数恢复、Session Title、Goal 模式、图片多模态理解、文本/图片消息入口语义、进度事件刷新节流边界、边界测试与回归（CI E2E 启动器、tool message 序列、默认 Bifrost 代理入 Traffic 等） |
