...
| [agent-builtin-tools-completeness.md](./agent-builtin-tools-completeness.md) | Agent Codex 内置工具完备性 | 8 | `exec_command` 短命令、交互任务 session + `write_stdin`、`view_image` data URL、`request_user_input` 不可交互边界、`tool_search` 发现新增核心工具、workspace all-features 编译回归、真实 Bifrost chat 端到端工具调用、本地 CI 静态门禁 |
