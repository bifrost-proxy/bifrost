...
| 2026-06-11 | TC-CWUT-07 | 跟进 GitHub Actions run `27355528937` 的 `Windows Unit Tests (x86_64)`，定位 `p1_tools_e2e::exec_command_tool_works_end_to_end` 仍要求短命 Windows TTY 初始输出包含 `True True`；本地执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-agent --test p1_tools_e2e exec_command_tool_works_end_to_end -- --nocapture`。 | 通过。非 Windows 保留 `isatty`/`exec-ready` 初始输出断言，Windows 改由后续 `write_stdin` 回显验证 TTY 会话可交互 |
