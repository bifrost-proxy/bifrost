...
### TC-REF-12：Windows ARM Rules CI 慢平台 timeout 回归
...
3. 检查 TLS readiness timeout 默认值：
   ...
   ```
4. 检查 GitHub Actions Windows rules CI 预算与分片：
   ```bash
   rg -n 'timeout-minutes: 150|rule_shard: "4/4"|fixture_timeout: "360"|tls_ready_timeout: "240"|BIFROST_E2E_RULE_SHARD_INDEX|BIFROST_E2E_FIXTURE_TIMEOUT: \$\{\{ matrix\.fixture_timeout \}\}' .github/workflows/ci.yml
   ```
5. 检查 rules runner 支持 CI 分片环境变量：
   ...
   ```
6. 在 GitHub Actions Windows ARM rules job 中触发 rules E2E。

**预期结果**：
- `test_rules.sh` 语法检查通过。
- 未显式设置 `BIFROST_E2E_TLS_READY_TIMEOUT` 时，TLS 拦截就绪等待继承 `BIFROST_E2E_FIXTURE_TIMEOUT`。
- Windows ARM rules job 使用 `BIFROST_E2E_FIXTURE_TIMEOUT=90` 时，不再因固定 30 秒 TLS readiness 误判所有 TLS 相关套件启动失败。
- Windows ARM rules job 使用 `BIFROST_E2E_SUITE_TIMEOUT=7200`，job 总 timeout 为 150 分钟，避免慢平台在完整 rules 套件完成前被 4800 秒 suite watchdog 收掉。
- 若 TLS 拦截真实初始化失败，日志仍输出具体 `TLS 拦截在 ${tls_timeout}s 内未就绪` 与数据目录路径，便于继续排查。

### TC-REF-13：TLS readiness probe 域名兼容回归

**操作步骤**：
1. 执行 rules harness 语法检查：
   ```bash
   bash -n e2e-tests/test_rules.sh
   ```
2. 检查 readiness probe 默认 URL 与自动注入的 host rule：
   ```bash
   rg -n 'bifrost-tls-probe\.invalid|__bifrost_tls_probe\.invalid' e2e-tests/test_rules.sh e2e-tests/tests/test_host_rule_path_rewrite.sh
   ```
3. 执行 probe 域名证书生成回归单元测试：
   ```bash
   cargo test -p bifrost-tls test_generate_for_probe_domain -- --nocapture
   ```
4. 执行独立 HTTPS host-rule fixture，验证 readiness probe 可以真实打通 TLS 握手与 HTTP 响应：
   ```bash
   bash e2e-tests/tests/test_host_rule_path_rewrite.sh
   ```
5. 推送后，使用 GitHub Actions PAT skill 跟踪 PR #111 的最新 run，确认 `E2E Rules (aarch64-pc-windows-msvc)` 不再卡在 probe 域名握手阶段。

**预期结果**：
- `test_rules.sh` 语法检查通过。
- `e2e-tests/test_rules.sh` 与 `e2e-tests/tests/test_host_rule_path_rewrite.sh` 仅使用合法的 `bifrost-tls-probe.invalid`，不再残留 `__bifrost_tls_probe.invalid`。
- `cargo test -p bifrost-tls test_generate_for_probe_domain -- --nocapture` 通过，说明动态证书生成器可为 probe 域名成功签发证书。
- `bash e2e-tests/tests/test_host_rule_path_rewrite.sh` 通过，说明 readiness probe 可完成 TLS 握手并拿到 2xx/3xx 响应，独立 HTTPS host-rule fixture 不退化。
- 推送后的 GitHub Actions Windows ARM rules job 不再因 probe 域名 TLS 握手问题持续打印 `TLS 拦截在 90s 内未就绪`；若仍失败，需归因为新的非域名问题并继续进入 fix→push→watch。
...
- 2026-05-07：通过。补充并执行 TC-REF-13；执行 `bash -n e2e-tests/test_rules.sh` 通过；执行 `rg -n 'bifrost-tls-probe\.invalid|__bifrost_tls_probe\.invalid' e2e-tests/test_rules.sh e2e-tests/tests/test_host_rule_path_rewrite.sh`，确认仅保留 `bifrost-tls-probe.invalid`，不再残留带下划线的旧 probe 域名；执行 `cargo test -p bifrost-tls test_generate_for_probe_domain -- --nocapture` 通过；执行 `bash e2e-tests/tests/test_host_rule_path_rewrite.sh` 通过，独立 HTTPS host-rule fixture 成功完成 TLS readiness 并返回 200。所有本地步骤均未使用 9900，代理启动参数保持 `--no-system-proxy`。
