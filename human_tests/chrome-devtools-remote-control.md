...
- 在 macOS CI 并行 shard 慢同步窗口下，新增记录最多允许约 15 秒出现在 Network 面板，不因固定 8 秒等待过短产生假失败。
...
- 2026-07-05：通过。根据 PR #309 CI run `28757358077` 的 macOS aarch64 shell shard 2 新失败复核 TC-CDP-06；失败点是 `devtools-refresh` 已可见但 `ant-modal-wrap` 在点击时拦截 pointer events。执行命令：`bash -n e2e-tests/tests/test_devtools_page_bridge_api.sh`、`rg -n "clickDevtoolsRefresh|dispatchVisibleClick\\(page.getByTestId\\('devtools-refresh'\\)" e2e-tests/tests/test_devtools_page_bridge_api.sh`，确认 Network refresh 按钮复用脚本内 DOM dispatch click 路径，避免 modal 遮罩/动画时序导致假超时。
