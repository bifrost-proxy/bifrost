...
- `upgrade -y --restart` 必须在新二进制落盘后、停止旧 daemon 前，用升级后的安装路径自动执行 `install-skill --tool all -y`；测试使用临时 HOME/USERPROFILE 和 `BIFROST_INSTALL_SKILL_SOURCE=embedded`，不得写入真实 AI tool skills 目录。
...
| 2026-06-16 | TC-IBOC-20 | `bash -n e2e-tests/tests/test_upgrade_local_restart_e2e.sh` + `rg -n 'Windows self-replacement runs in a deferred helper|assert_post_upgrade_skills_installed' e2e-tests/tests/test_upgrade_local_restart_e2e.sh human_tests/install-binary-one-click.md` | PASS：静态验证 Windows upgrade restart 脚本不再在 upgrade 命令刚返回时检查 skill 文件，而是在 Admin API ready 后检查 deferred helper 已安装 primary/remote skills；本地未执行真实 upgrade restart，避免启动 Tray 或打开 Sync 登录页，完整 Windows 真实链路交由 GitHub Actions `E2E Shell (x86_64-pc-windows-msvc)` 验证。 |
