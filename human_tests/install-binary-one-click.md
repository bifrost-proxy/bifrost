...
| 2026-08-11 | TC-IBOC-27 | `BIFROST_BIN="$PWD/target/debug/bifrost" bash e2e-tests/tests/test_upgrade_admin_api_restart_e2e.sh` | PASS（CI 模块门禁修复后复测）：15/15；worker CLI 0 秒内返回，progress=`completed`/source=`admin`，daemon PID `94458 -> 95490`，新进程使用升级后的安装路径；already-latest 后台重启也成功完成。 |
