...
| 2026-07-03 | TC-IBOC-20 | `SKIP_BUILD=true BIFROST_BIN=target/release/bifrost BIFROST_DISABLE_TRAY=1 BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 bash e2e-tests/tests/test_upgrade_admin_api_restart_e2e.sh` | PASS：Admin API upgrade restart E2E 14/14 通过。脚本基于当前 workspace 版本 `0.0.136` 构造本地 `0.0.137` release archive，`/_bifrost/api/system/version-check` 返回可更新，`POST /api/system/upgrade` 进入 checking/downloading 并最终 completed，source=admin；真实替换安装路径二进制，旧 daemon PID 被新 PID 替换，升级后 daemon 从 upgraded install path 运行，stop 后端口释放；already-latest background upgrade 分支也能重启并写 completed progress。 |
