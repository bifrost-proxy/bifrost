...
- 2026-06-30：针对本轮 workspace all-features 中 `status_for_config_ignores_version_only_mismatch` 并行/单独运行版本来源不一致的测试稳定性问题，执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-core system_proxy_launchd::tests::status_for_config_ignores_version_only_mismatch --lib -- --nocapture`，结果 1/1 PASS。该用例继续验证 TC-CSP-16 的核心语义：LaunchDaemon `program`、`data_dir`、`RunAtLoad` one-shot 结构一致时，仅 installed/current version 不一致不应触发 `needs_upgrade`。
