...
- CI run `26889180033` 中 `E2E Rules (x86_64-pc-windows-msvc, shard 2/4)` 在 GitHub runner 上超时失败且没有上传失败日志；已将两个语法型 Replay rules fixture 从 Windows rules shard 中排除，保留 Linux rules CI 与 `test_replay_rules.sh` 的真实覆盖，并用分片模拟确认 Windows rules shard heavy fixture 分布从 `7/9/6/5` 调整为 `6/7/6/6`。
