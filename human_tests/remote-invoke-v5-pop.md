...
Remote Invoke v5 将调用方敏感路径从 v4 的 caller fingerprint / grant id 明文信任模型，升级为 Ed25519 Proof-of-Possession（PoP）签名、一次性 `claim_token`、短期 `grant_session_token` 和调用方公钥指纹绑定。本文档覆盖 v5 配对、授权领取、授权查询、调用打开、撤销、旧 v4 caller 路径拒绝、nonce 重放防护、SSE 多订阅者与日志脱敏验证。
...
- 如保留日志用于排查，检查完成后可删除 `/tmp/bifrost-ri-v5-pop-human-test.log`。
