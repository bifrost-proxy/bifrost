...
| TC-V5-04 | 旧 v4 caller 敏感路由已移除 | 执行 `source ~/.zshrc; cd /Users/eden/work/github/bifrost/packages/bifrost-sync-server; pnpm exec vitest run src/__tests__/remote-invoke-security.test.ts`，重点覆盖 `does not register legacy v4 caller-sensitive endpoints` 与 `does not register legacy v4 caller openCall`。 | `/v4/remote-invoke/pairings/start`、`/v4/remote-invoke/pairings/:id/watch`、`/v4/remote-invoke/grants/reusable`、`DELETE /v4/remote-invoke/grants/:id`、`/v4/remote-invoke/calls/open` 均返回 404 `remote invoke endpoint not found`，不再注册 legacy caller handler。 | PASS。2026-06-30 定向执行通过，`1` 个 test file、相关 legacy v4 caller 路由移除断言通过。 |
