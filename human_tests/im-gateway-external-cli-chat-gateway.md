...
预期结果：
1. progress card 在运行中展开“执行过程”，按事件顺序展示公开状态/思考摘要和 `exec_command` 工具摘要；工具详情行默认折叠，展开后能看到参数、耗时和输出预览。
2. progress card 完成后最终结论在卡片顶部可见，过程信息和状态面板默认折叠，但可以手动展开查看完整过程；卡片不再停留在 running。
