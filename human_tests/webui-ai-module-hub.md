...
4. 在 Agent Configuration 确认 Runners 位于顶部，其后依次为 General、Skills，且 General 中没有 `Enable Agent` 开关。
5. 每页点击左上角“AI Home”。

预期：每个模块都有独立 URL，返回操作均回到四卡首页且保留系统侧栏。
...
3. 操作状态筛选并观察 URL。
4. 检查行内不存在链接或点击下钻；检查网络请求。

预期：筛选写入 URL；列表仅展示允许的摘要字段；不请求工作目录、消息、思考、工具调用、历史路径和诊断详情。
...
预期：首页卡片变为单列；运行记录变为不可点击的移动端摘要卡片，无横向表格溢出。

### TC-AIH-05 亮色与暗色主题

1. 分别设置系统主题为 light 和 dark 后打开 AI 首页。
2. 检查页面、卡片、文字、边框、状态 Tag 与交互焦点。
