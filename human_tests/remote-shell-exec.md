...
| TC-RSE-24 | ✅ PASS | 2026-06-11 本地 targeted 复测暴露 `remote_invoke::executor::tests::test_execute_shell_exec_streams_stdout_before_exit` 在重编译/高负载环境下第一段 stdout chunk 偶发 840ms 才被测试 sink 观察到，旧断言 `first chunk < 250ms` 误把调度抖动判成产品失败。该用例目标是确认 stdout 按 chunk 流式到达，且第一段早于带 intentional delay 的第二段，而不是验证绝对 250ms SLA；修复后断言改为 `received[0].1 < received[1].1` 并保留第二段 `>=250ms` 延迟检查，仍能防止“等进程退出后一次性返回”的回归。使用隔离 target 执行 `CARGO_TARGET_DIR="$PWD/target-codex-verify" SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin remote_invoke::executor::tests::test_execute_shell_exec --lib -- --nocapture`，7 个 executor shell exec 测试全部通过。 |
| TC-RSE-25 | ✅ PASS | 2026-06-11 隔离 target 复跑同一组 executor 单测时，`remote_invoke::executor::tests::test_execute_shell_exec_pty_reports_isatty_true` 在高负载冷编译后偶发命中夹具策略 `max_timeout_ms=5000`，错误为 `shell.exec PTY wall-clock timeout after 5000 ms (policy 'pty-shell')`。该用例目标是验证 PTY stdin/stdout 均为 TTY，不是 timeout 边界；修复后仅将该测试 policy 调宽到 15000ms，产品默认 timeout 和独立 wall-clock timeout 测试不变。继续复跑 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin remote_invoke::executor::tests::test_execute_shell_exec --lib -- --nocapture`，确认 PTY、streaming stdout、stdin stream 和 wall-clock timeout enforcement 同组通过。 |
| TC-RSE-26 | ✅ PASS | 2026-06-11 Windows CI 暴露 `test_execute_shell_exec_streams_stdout_before_exit` 在 PowerShell/Windows runner 上仍可能超过 15 秒才观察到测试 sink 的第一段 chunk，且该测试只验证“退出前流式输出”相对顺序，不验证 Windows 特定 shell 行为。修复后该单测仅在非 Windows 平台执行；Windows 仍保留 shell detection、stdin stream 转发和 CRLF 归一化回归。执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin remote_invoke::executor::tests::test_execute_shell_exec --lib -- --nocapture`，本机 7 个 executor shell exec 测试通过；Windows 专属 cfg 由远端 CI 验证编译与剩余测试。 |
