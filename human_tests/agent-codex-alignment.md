...
- mock 在不主动轮询期间等待长任务结束，再通过 `write_stdin` 空输入轮询同一个 `session_id`，应返回 `WATCH_DONE` 与 `exit_code=0`，证明后台 watcher 已感知进程真实结束。
- `/agent/chat` 最终响应 `success=true`，包含 `CHAT_CODEX_ALIGNMENT_OK`。
- `tool_calls` 顺序为 `exec_command`、`exec_command`、`update_plan`、`set_title`、`tool_search`、`list_mcp_resources`、`read_mcp_resource`、`exec_command`、`write_stdin`，全部成功。
...
- 总耗时低于 7000ms，证明两个 `exec_command` tool 在同一批内并发执行，同时覆盖后台长任务结束后由 `write_stdin` 获取最终 exit code。
...
| TC-ACA-07 | 通过 | 2026-05-12 | `ADMIN_PORT=18917 MOCK_HTTP_PORT=18918 bash e2e-tests/tests/test_agent_codex_alignment_chat_api.sh` 真实构建路径功能通过，首次因新增后台等待后的阈值仍按旧 3800/4200ms 预算而失败；调整为 7000ms 后，`SKIP_BUILD=true ADMIN_PORT=18917 MOCK_HTTP_PORT=18918 bash e2e-tests/tests/test_agent_codex_alignment_chat_api.sh` 通过，耗时 5050ms。脚本以临时数据目录和 `--no-system-proxy` 启动真实 Bifrost 服务，通过真实 `/agent/chat` 覆盖默认 prompt 不泄露兼容实现说明、MCP resource canonical tools、deferred MCP tools 通过 `tool_search` 发现但不直接暴露、`update_plan`、`set_title`、MCP resource read、并发 `exec_command` tool batch、history 顺序回填、后台长任务结束后由 `write_stdin` 获取最终 `exit_code=0`。 |
