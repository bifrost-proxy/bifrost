...
验证 Bifrost Agent 在不实现权限、沙箱、审批能力的前提下，继续对齐核心工具命名、MCP resource 暴露、旧 shell alias 拒绝、事件驱动 turn runtime 和并发工具批调度行为；同时确认默认 prompt 不暴露内部实现说明。
...
### TC-ACA-03 历史 shell alias 不可见且不可执行
...
- 测试通过。
- 默认模型可见工具列表不包含 `shell_command` 和 `local_shell`。
- `shell_command` 旧名称执行失败并返回 `unknown tool`。
...
- 测试通过。
- `tool_search`、Goal、`update_plan`、`set_title`、`switch_workdir` 等状态型工具判定为 ordered。
- 普通本地工具仍判定为 parallel。
...
- mock 捕获到首个模型请求：
  ...
  - tools 包含 `tool_search`、`list_mcp_resources`、`list_mcp_resource_templates`、`read_mcp_resource`。
  - tools 不包含 `shell_command` / `local_shell`。
  - tools 不直接包含超过阈值后被 deferred 的 `mcp_mcpfixture__sample_tool_*`。
- 模型返回同一批 tool calls：两个 2 秒 `shell`、一个 `update_plan`、一个 `set_title`、一个 `tool_search`、`list_mcp_resources`、`read_mcp_resource`。
- `/agent/chat` 最终响应 `success=true`，包含 `CHAT_CODEX_ALIGNMENT_OK`。
- `tool_calls` 顺序为 `shell`、`shell`、`update_plan`、`set_title`、`tool_search`、`list_mcp_resources`、`read_mcp_resource`，全部成功。
...
- 总耗时低于 3800ms，证明两个 2 秒 shell tool 在同一批内并发执行，而不是顺序执行。
...
| TC-ACA-03 | 通过 | 2026-05-11 | `cargo test -p bifrost-agent tools::tests::unknown_shell_aliases_are_rejected` 通过：1 passed；确认旧 shell alias 不可见且执行失败。 |
...
| TC-ACA-06 | 通过 | 2026-05-11 | `cargo test -p bifrost-agent turn_runtime` 通过：2 passed；状态型工具保持 ordered，普通本地工具可 parallel。 |
| TC-ACA-07 | 通过 | 2026-05-11 | `ADMIN_PORT=18917 MOCK_HTTP_PORT=18918 bash e2e-tests/tests/test_agent_codex_alignment_chat_api.sh` 通过；脚本先 `cargo build --bin bifrost`，再以临时数据目录和 `--no-system-proxy` 启动真实 Bifrost 服务，通过真实 `/agent/chat` 覆盖默认 prompt 不泄露兼容实现说明、MCP resource canonical tools、deferred MCP tools 通过 `tool_search` 发现但不直接暴露、`update_plan`、`set_title`、MCP resource read、并发 shell tool batch 与 history 顺序回填。 |
| TC-ACA-08 | 通过 | 2026-05-11 | CI 回归验证：`SKIP_BUILD=true ADMIN_PORT=18933 MOCK_HTTP_PORT=18934 bash e2e-tests/tests/test_agent_codex_alignment_chat_api.sh` 通过，确认 shell E2E 在 CI 预构建 `target/release/bifrost` 场景下不再错误回退到缺失的 `target/debug/bifrost`。 |
