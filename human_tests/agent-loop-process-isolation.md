...
| 2026-06-16 | TC-ALPI-03 | `bash -n e2e-tests/tests/test_agent_worker_process_isolation.sh` + `rg -n 'disconnect stream did not emit run_started|disconnect sse' e2e-tests/tests/test_agent_worker_process_isolation.sh` | 通过：静态验证 SSE disconnect 回归脚本在 kill curl 前先等待 disconnect stream 收到 `run_started`，避免服务端尚未登记断开处理器时提前断开导致 worker 继续跑满慢速 mock；失败日志会额外输出 disconnect/external SSE。因用户明确要求本地测试不得启动 Tray/登录页，本地未执行真实 Bifrost E2E，真实链路由 GitHub Actions Linux shell E2E 验证。 |
| 2026-06-17 | TC-ALPI-03 | 本地 Mac 执行 `SKIP_BUILD=true bash e2e-tests/tests/test_agent_worker_process_isolation.sh`（独立端口/数据目录）连续 2 次 | 通过：2/2 `[agent-worker-process-isolation-e2e] PASS`。按 design/im-gateway-agent.md 收敛断开语义：SSE client disconnect 不再杀 worker（worker 继续后台执行），改为断开后断言 worker 仍存活、再用显式 `/stop` 回收后台 worker 并校验 worker 退出与 Admin API 存活。修正了沿用旧 main「断开即杀」断言导致的 Linux/Windows shell E2E 失败。|
