...
### TC-RFRD-04：单条规则允许不安全上游 HTTPS 证书
...
### TC-RFRD-05：未配置 `upstreamUnsafeSsl` 的同上游保持严格校验

操作步骤：

1. 复用 TC-RFRD-04 的自签名 HTTPS echo 上游服务和 Bifrost 进程。
2. 通过 Bifrost 代理访问：
...
### TC-RFRD-06：Network `.bifrost` 导出保留实际转发诊断字段
...
### TC-RFRD-07：通用并行规则 runner 不重复执行专用回归 fixture
...
- 输出不出现 `过滤器规则验证未实现`。
- 专用脚本 `e2e-tests/tests/test_rule_filter_routing_diagnostics.sh` 仍负责执行 TC-RFRD-01 到 TC-RFRD-06 的真实请求断言。
