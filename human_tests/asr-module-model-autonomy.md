...
| 2026-05-29 | TC-ASR-AUTO-04 | CI `E2E Shell (Linux, shard 1/3)` 暴露 Linux 上 Qwen3-ASR runtime unsupported 时仍断言共享启动成功；已更新脚本为平台感知断言，并执行 `SKIP_BUILD=true BIFROST_BIN=$PWD/target/debug/bifrost BIFROST_ASR_MODEL_AUTONOMY_E2E_PORT=18991 bash e2e-tests/tests/test_asr_model_autonomy.sh` | 通过；Apple Silicon macOS 要求同模型跨 owner 复用成功，Linux/Windows 等 unsupported 平台要求返回明确 unsupported 且不假装 managed runtime |
