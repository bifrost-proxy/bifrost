...
- 第 3 步 CLI 输出包含默认远端 URL `"${BIFROST_DEFAULT_REMOTE_URL}"`
- 第 4 步 help 输出包含 token 获取地址 `${BIFROST_DEFAULT_REMOTE_URL}/v4/sso/token-login`
...
- 默认 Provider 场景输出包含 `Sync session token for non-interactive login; get one at ${BIFROST_DEFAULT_REMOTE_URL}/v4/sso/token-login`。
...
**预期结果**：
- `bifrost login --help` 输出包含 `Equivalent to \`bifrost sync login\``、`--token`、`--url` 和默认 token 获取链接 `${BIFROST_DEFAULT_REMOTE_URL}/v4/sso/token-login`。
