...
- 手机公开 Availability Check 页在未安装或未信任 CA 时可以每秒重试探测，但证书失败提示、下载按钮、下一步说明必须保持 DOM 稳定，不能在窄屏手机上反复闪烁或错位。
...
3. 等待至少 5 秒，观察证书状态区域、`Download Bifrost CA` 按钮、下一步说明。
4. 记录 `#result`、`#next` 的 DOM mutation 次数或用肉眼确认页面是否稳定。

**预期结果**：
- 页面先展示 `Browser HTTPS probe failed` 和 `Download Bifrost CA`。
- 后续每秒轮询不会把相同证书失败提示、下载按钮、下一步说明反复替换。
