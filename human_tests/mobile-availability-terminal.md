...
- 终端面板生成二维码时不应提前常驻占用 probe 端口；目标设备真实 GET 打开 Availability Check landing page 后，才按需启动对应 host/CA 的 probe listener。已有 probe listener 健康时必须复用同一个端口，不因多设备并发打开或页面刷新而反复重启；只有旧 listener 已停止时才自愈重建。
