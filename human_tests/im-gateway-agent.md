...
- **预期结果**:
  ...
  - `AssistantDelta` 和运行中的 `AssistantFinal` 进入执行过程并归并为可读思考，不出现逐字符/逐 token 独立条目，也不与最终输出重复。
...
- **执行记录（2026-05-10）**:
  ...
  - 2026-07-11 回归修复：assistant stream 改为连续片段/累计快照归并；专项 shell E2E 验证过程区同时保留可读思考、工具与状态，最终正文只展示一次。
...
- **操作步骤**:
  ...
  3. 检查通知正文使用 Claude Code settings 的当前模型和 effort。
- **预期结果**:
  - `Runner Type` 为 `claude_code`，`Runner ID` 为 `Claude-Code`。
  - `Model` 显示 `opus（claude settings）`。
  - `Reasoning Effort` 显示 `low`，不再是 `N/A`。
