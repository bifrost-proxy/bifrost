...
1. 准备一个唤醒音频样本。真实测试时用本人说出唤醒词的音频；自动化回归可用隐藏 `--phrase` 固定后端识别结果：
   ```bash
...
- `binding.phrase=打开录音`；真实音频路径下该短语来自后台 ASR 识别结果，用户不手动编辑。
...
1. 打开 ASR 页面：
   ...
   ```
2. 在 `Voice Wake Actions` 面板点击 `Record Wake Audio`，说出 `测试唤醒`，然后停止录音。
3. 等待后台 ASR 识别完成，确认 `Wake phrase` 自动显示为 `测试唤醒`。
...
- `Wake phrase` 展示在录音区域下方，为只读，用户不能手动输入或修改。
...
2. 查询状态：
   ```bash
   curl -sS http://127.0.0.1:18892/_bifrost/api/voice/wake/status
   ```
3. 查询事件：
   ```bash
   curl -sS http://127.0.0.1:18892/_bifrost/api/voice/wake/events
...
- `listener.last_transcript` 包含 `打开录音`。
- `listener.last_speaker_profile_id=speaker_other`。
- `listener.last_error` 包含 `speaker verification failed`。
- events 数量不会因为这次非绑定声纹而增加。
...
- 重新启动 Bifrost 后可再次录音识别。

实际结果：

- 2026-05-27：通过。确认 18892 无监听进程时，录音识别失败原因是后端不可达；WebUI 已将原始 `Failed to fetch` 收敛为明确的 Bifrost 后台不可达提示。
