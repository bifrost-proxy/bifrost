...
### TC-ADA-20 回归：ChatGPT Web 单日期失败不阻断后续日报生成

操作步骤：
1. 执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin daily_agent_report_gate_excludes_known_failed_entries_from_missing_reports --lib -- --nocapture`。
2. 执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin daily_agent_entry_failure_summary_lists_failed_dates_and_targets --lib -- --nocapture`。
3. 执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-admin daily_agent_chatgpt_web_timeout_is_bounded_below_outer_timeout --lib -- --nocapture`。
4. 执行 `bash e2e-tests/tests/test_chatgpt_web_shared_profile.sh`，确认 ChatGPT Web Daily Agent 源码哨兵包含 `ASR daily agent entry failed; continuing with remaining entries` 和 `partial_success`。
5. 对照 `design/asr-daily-agent-failure-isolation.md`，确认全局失败和单日期失败的边界符合设计。

预期结果：
- 已知失败日期的 report target 会从 missing report gate 中排除，后续已生成日期仍可写入 processed state。
- 失败摘要包含失败日期、报告目标和错误摘要，方便 UI/API 展示失败原因。
- 默认 Daily Agent timeout 为 1 小时；ChatGPT Web 内部 timeout 使用 30 秒 headroom，即默认 3570 秒。
- ChatGPT Web 单日期失败被记录后继续处理剩余 changed entries；同时只在至少一个 report 成功时把 run 标记为 `partial_success`。
- 非 ChatGPT Web 的整批 runner 失败仍保持全局失败，不伪造按日期继续执行。

### TC-ADA-21 回归：Daily Docs 显式重跑不受历史状态和内容哈希影响

操作步骤：
1. 执行 `pnpm --dir web exec playwright test tests/ui/asr-daily-agent-runner.spec.ts --grep "force reruns all or one Agent" --workers=1`，测试服务必须使用 Playwright 隔离环境，不得向正式 9900 服务提交 run 请求。
2. 在 Daily Docs 行点击 `Run All Agents`，检查被拦截的 `POST /daily-agent/run` URL。
3. 打开同行下拉菜单并选择 `Run daily_report`，再次检查请求 URL。
4. 将 mock 后端响应改为 `status=already_running` 后再次点击主按钮，观察页面消息类型和文案。

预期结果：
- `Run All Agents` 请求固定包含目标行 `date` 和 `force=1`，不携带 `agent_id`；即使该日期历史上已经成功、失败或 source hash 未变化，也不会被 ChangePlanner 的 `unchanged` 跳过。
- `Run daily_report` 请求固定包含同一 `date`、`agent_id=daily_report` 和 `force=1`，只强制重跑所选 Agent。
...
### TC-ADA-23 回归：长时间 ChatGPT Web 请求不被 browser worker 的 30 分钟默认上限截断
...
7. 如果旧 run 已进入缺失上下文的 provisional 对话，先通过 `/worker-jobs/{job_id}/cancel` 精确停止对应 browser job，确认 Daily Agent run 变为 `failed`，不得停止整个 worker kind 或无关 external CLI 会话。
8. 轮询新 `run_id`；如果触发 conversation 丢失后的纠偏重放，检查日志确认新对话获得完整原始上下文，且 provisional handoff 不会重复发送。
9. 等待 `success`，确认目标报告非空、标题与日期/Agent 契约一致，并检查 `daily_agent_processed.json` 写入同一新 `run_id`；再串行触发同日期下一个 Agent。
10. 如果 AGENTS.md 或已有输出含历史固定日期，确认动态运行契约位于整个 prompt 末尾；最终报告必须使用本次运行日期，不得保留旧日期或日期冲突说明。
