...
```
4. 打开 `http://127.0.0.1:<port>/_bifrost/settings?tab=tray`，确认 Settings 中存在独立 Tray tab，`Tray Icon` 和 `Show System Stats` 默认打开；分别在桌面和移动端 viewport 截图确认图标、文案、开关位置没有遮挡、错位或横向溢出。
5. 在 macOS 上通过 System Events 点击当前 `bifrost` 菜单栏图标，读取原生菜单文本；在 Windows VM 上通过真实交互 session 启动同一测试脚本并打开 notification area 菜单。
6. 展开桌面原生 tray 菜单后连续截取至少 3 张截图，每张间隔 1 秒，确认 `System:` 与 `Network:` 两排真实展示，且网络速率按 1 秒刷新。
7. 关闭 `Show System Stats` 后再次展开托盘菜单，并截图确认两排系统状态消失。
8. 再次打开 `GET /_bifrost/api/config/tray`，确认 `show_system_stats` 与 Settings 开关一致。
...
- 桌面原生 tray 截图中必须看到 `System:` 与 `Network:` 两排；连续 1 秒间隔截图中的网络速率应随系统流量变化刷新，菜单不应闪退或被刷新关闭。
