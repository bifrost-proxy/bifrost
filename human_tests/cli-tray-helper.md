...
| 2026-06-22 | TC-TH-30-REG-MEM-01 | 针对用户反馈 macOS tray `MEM` 显示 75%-80%，但系统实际内存占用远低于该值执行口径回归。审查确认旧实现使用 `total - free - speculative` 近似物理非空闲页，会把 inactive / file-backed cache 也算成已用；本机对照 `sysctl -n hw.memsize` 为 `68719476736`，`vm_stat` 显示大量 inactive/file-backed pages，`top` 的 `PhysMem used` 同样会落在偏高的非空闲口径。修复为 macOS `MEM` 使用偏用户感知的即时压力口径：`internal - purgeable + wired`，排除 file-backed / inactive cache / 可回收缓存，也不把 long-lived compressed store 当成当前活跃消耗；极端字段为空时才 fallback 到非空闲页估算。新增单测覆盖 file cache 不参与 MEM 百分比，以及压力字段为空时 fallback 不返回 0。 | 通过。执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli mac_memory_used_pages --lib --all-features -- --nocapture` 通过 2/2；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli system_stats --lib --all-features -- --nocapture` 通过 29/29；执行 `BIFROST_BIN="$PWD/target/debug/bifrost" BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 bash e2e-tests/tests/test_tray_system_stats_config.sh` 通过 63/63。本机原始页数对照旧口径约 `84.2%`、新压力口径约 `51.3%`；最终安装当前 release 到 `/Users/eden_studio/.cargo/bin/bifrost` 与 `/Users/eden_studio/.local/bin/bifrost` 后重启真实 9900 服务，主进程 PID `84343`、tray PID `84388`，AX description 为 `Bifrost: C15% | M40% | D30% | ↑114 K/s ↓1.9 M/s`；截图 `/tmp/bifrost-tray-mem-pressure/topbar.png` 可见菜单栏状态项显示 `M40%` 且菜单正常展开。 |
