...
- 包含以下菜单项：Open Traffic、Open Rules、Open Settings、Copy HTTP Proxy
- 点击 Open Settings 打开 `http://127.0.0.1:8801/_bifrost/settings`
- 默认菜单不展示 Copy SOCKS5 Proxy；SOCKS5 地址仍可通过自定义菜单模板 `{socks5_proxy}` 使用
...
- 鼠标悬停项保持可见且可高亮
- macOS native 菜单实际展开期间，即使超过 3 秒保护窗口，也不能因为后台数据刷新、System Proxy 刷新或状态项宽度变化而关闭弹窗；菜单关闭后允许补应用被延迟的快照刷新
...
- 缓存为空或 Admin API 快照来不及返回时，菜单项必须存在但禁用，不能根据本机 OS 系统代理状态猜测勾选态，也不能发起启用/停用写请求
- 后台第一次快照会请求一次当前实例 `/api/proxy/system` 补齐状态
...
### TC-TH-20-REG-01: Stop 后同一托盘 Start 成功必须刷新 Running 菜单快照

**操作步骤：**
1. 使用当前 `target/debug/bifrost` 在 9900 端口启动真实 macOS 服务和托盘，并等待菜单显示 Running。
2. 展开托盘菜单，记录 `Open Traffic`、`Open Rules`、`Open Settings`、`Copy HTTP Proxy`、`Rules`、`Stop Bifrost`、`System Proxy` 与底部 Running 状态。
3. 点击 `Stop Bifrost`，立即展开菜单记录进行中状态，再等待服务停止后重新展开菜单。
4. 点击同一个托盘 helper 中的 `Start Bifrost`，等待最多 10 秒后重新展开菜单。
5. 同时用 `lsof -nP -iTCP:9900 -sTCP:LISTEN`、`GET /_bifrost/api/proxy/system`、`GET /_bifrost/api/rules/active-summary` 验证真实服务、系统代理和规则状态。

**预期结果：**
- 点击 Stop 后短暂显示 `Stopping Bifrost...`，最终显示 `Start Bifrost` 与 `Bifrost: Disconnected`，9900 不再监听，OS 系统代理不残留 Bifrost 配置。
- 点击 Start 后必须显示 `Starting Bifrost...` 或禁用中的启动项，不能表现为点击无反应。
- Start 成功后菜单必须刷新为 Running 快照：包含 `Open Traffic`、`Open Rules`、`Open Settings`、`Copy HTTP Proxy`、`Rules`、`Stop Bifrost`、`System Proxy`，底部显示 `Bifrost: Running on 127.0.0.1:9900`。
- Start 成功后不得停留在 `Bifrost: Unknown`、`Bifrost: Disconnected` 或仅有 `Stop Bifrost` 的旧快照。
- 9900 端口真实监听，Admin API 可访问，Rules active-summary 与菜单 `Rules` 状态一致；System Proxy 菜单状态与 Admin API / OS 状态一致。
- 托盘 Start 监控线程不得等待长驻 daemon 子进程退出后才刷新菜单，runtime ready 后应立即刷新同进程菜单快照，并异步回收子进程。
- 点击 `System Proxy` 后必须立即显示 `Enabling System Proxy...` 或 `Disabling System Proxy...`，并禁用重复点击，直到 Admin API / OS 状态收敛后恢复为 `System Proxy`。
- 点击 Rules 候选后必须立即显示 `Rules: Applying <rule>` 或 `Rules: Disabling <rule>`，目标规则项显示 `Applying <rule>...` / `Disabling <rule>...`，其它规则与 System Proxy 暂时禁用，直到 active-summary 收敛后恢复。
...
| 日期 | 用例 | 执行方式 | 结果 |
| --- | --- | --- | --- |
| 2026-06-23 | TC-TH-20 / TC-TH-20-REG-01 / TC-TH-06 / TC-TH-13 / TC-TH-14-REG-02 | 针对用户反馈“服务其实已经 start 起来了，但托盘仍显示旧状态/Unknown”执行 macOS 真实 9900 回归。使用当前 `target/debug/bifrost` 以 `BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 ./target/debug/bifrost start --daemon -y -p 9900 --system-proxy --skip-cert-check` 启动服务；初始菜单通过 System Events 读取到 `Open Traffic`、`Open Rules`、`Open Settings`、`Copy HTTP Proxy`、`Rules: None`、`Stop Bifrost`、`System Proxy`、`Bifrost: Running on 127.0.0.1:9900`，`GET /api/proxy/system` 返回 `enabled=true host=127.0.0.1 port=9900 managed_by_bifrost=true configured_enabled=true`，`scutil --proxy` 显示 HTTP/HTTPS 均指向 `127.0.0.1:9900`。点击 `System Proxy` 关闭后 API 返回 `enabled=false configured_enabled=false` 且 OS HTTP/HTTPS 均为 0，菜单仍保持 Running/Stop/Rules；再次点击开启后 API 与 OS 均恢复到 `127.0.0.1:9900`。点击 Rules 子菜单中的个人规则 `a` 后 `active-summary` 为 total 1 且 `name=a`，菜单变为 `Rules: a`；再次点击 `a` 后 `active-summary` total 0，菜单回到 `Rules: None`。点击 `Stop Bifrost` 后立即菜单显示 `Stopping Bifrost...` / `Bifrost: Stopping...`，最终显示 `Start Bifrost` / `Bifrost: Disconnected`，9900 无监听且 OS 系统代理关闭。随后点击同一托盘中的 `Start Bifrost`，最终菜单刷新为 `Open Traffic`、`Open Rules`、`Open Settings`、`Copy HTTP Proxy`、`Rules: None`、`Stop Bifrost`、`System Proxy`、`Bifrost: Running on 127.0.0.1:9900`，未再停留在 `Unknown` 或旧快照；`lsof` 确认 9900 监听，`GET /api/proxy/system` 返回 `enabled=false configured_enabled=true`（托盘 Start 继承 `--no-system-proxy` 的安全默认，未擅自打开 OS 代理），`active-summary` total 0。随后补充异步中间态验证：System Proxy action 开始后菜单项必须立即显示 `Enabling System Proxy...` / `Disabling System Proxy...` 且不可重复点击；Rules action 开始后顶层显示 `Rules: Applying a` 或 `Rules: Disabling a`，目标规则项显示 `Applying a...` / `Disabling a...`，其它 Rules 候选和 `System Proxy` 暂时禁用。相关自动验证执行 `cargo test -p bifrost-cli commands::tray -- --nocapture` 通过 lib/main 各 147/147，`cargo build --bin bifrost` 通过。 | 通过。同一托盘 Stop/Start、System Proxy 开关、Rules 启用/取消均在真实菜单、Admin API、OS 状态和端口监听之间闭环；Start 成功后菜单立即进入 Running 快照，不再因状态获取异常显示旧状态；System Proxy 与 Rules 异步动作有明确中间态和重复点击保护。 |
| 2026-06-23 | TC-TH-06 / TC-TH-14 / TC-TH-14-REG-02 / TC-TH-14-REG-03 | 针对用户要求“所有状态切换后都验证其他所有状态”执行 macOS 真实托盘矩阵。使用当前 `target/debug/bifrost` 重启 9900 服务，最终运行 PID `48257`，启动参数 `--system-proxy --skip-cert-check`。系统代理关闭：通过 System Events 点击 `System Proxy` 后，`GET /api/proxy/system` 返回 `enabled=false configured_enabled=false`，`scutil --proxy` 显示 `HTTPEnable=0 HTTPSEnable=0`，菜单 `system_proxy_mark=missing value`，同时 `Rules: None`、`stop_present=true`、`start_present=false`。系统代理开启：再次点击后 API 返回 `enabled=true host=127.0.0.1 port=9900 managed_by_bifrost=true configured_enabled=true`，`scutil` 显示 HTTP/HTTPS 均指向 `127.0.0.1:9900`，菜单 `system_proxy_mark=✓`，同时 `Rules: None`、Stop/Start 状态保持正确。规则启用/停用：在 active rules 为 0 的基线下点击 My Rules 中的 `a`，`active-summary` 返回 total 1 且 `name=a`，菜单变为 `Rules: a` 且 `a_mark=✓`，系统代理仍 `✓`、Stop present；再次点击 `a` 后 `active-summary` 返回 total 0，菜单回到 `Rules: None` 且 `a_mark=missing value`，系统代理仍 `✓`。Stop 验证：点击 `Stop Bifrost` 后 9900 API 不可达、`lsof` 无监听、`scutil` 显示 `HTTPEnable=0 HTTPSEnable=0`，托盘菜单 `stop_present=false`、`start_present=true`、`rules_name=Rules: None`，且 `system_proxy_present=false` 不再残留旧勾；随后重新启动 9900，确认 `GET /api/proxy/system` 为 enabled=true，`active-summary` total 0。相关单测执行 `cargo test -p bifrost-cli test_menu_stopped_state -- --nocapture` 通过、`unset BIFROST_SYSTEM_PROXY_RECONCILE_INTERVAL_SECS && cargo test -p bifrost-cli system_proxy -- --nocapture --test-threads=1` 通过 lib/main 各 44/44 且 cli_commands 2/2、`cargo test -p bifrost-cli load_rules_from_admin_marks_active_personal_and_group_rules -- --nocapture` 通过、`cargo test -p bifrost-cli test_active_rule_targets_match_desired_state -- --nocapture` 通过。 | 通过。System Proxy、Rules、Stop/Start 三类动作均在真实菜单、Admin API 和 OS 状态之间闭环；每次切换后同时验证其他状态未被误改。 |
| 2026-06-23 | TC-TH-14-REG-02 / TC-TH-14-REG-03 | 针对用户连续复现的 macOS tray `System Proxy` 未勾选、点击仍发启用请求，以及 `Rules: None` 回归执行修正后真实验证。本轮方案收敛为 API-only：tray 启动时用当前实例 Admin API 拉完整菜单快照（rules reference-candidates、group、active-summary、system proxy），不读取本机 OS 系统代理做状态兜底；Admin API 未返回系统代理状态时 `System Proxy` 菜单项存在但禁用；若 API 返回 `configured_enabled=true` 但 live 状态尚未收敛，后台 3 秒轮询继续 recheck，避免缓存启动期 false。执行 `cargo test -p bifrost-cli test_initial_menu_snapshot_loads_rules_and_system_proxy_from_admin_api -- --nocapture`、`cargo test -p bifrost-cli test_load_system_proxy_state_rechecks_configured_but_inactive -- --nocapture`、`cargo test -p bifrost-cli test_background_menu_refresh_rechecks_configured_inactive_system_proxy_cache -- --nocapture` 均通过；执行 `unset BIFROST_SYSTEM_PROXY_RECONCILE_INTERVAL_SECS && cargo test -p bifrost-cli system_proxy -- --nocapture --test-threads=1` 通过 lib/main 各 43/43 且 cli_commands 2/2；执行 `cargo test -p bifrost-cli load_rules_from_admin_marks_active_personal_and_group_rules -- --nocapture` 通过。随后用当前 `target/debug/bifrost` 重启真实 9900 服务（PID 21989，tray PID 22027，启动参数 `--system-proxy --skip-cert-check`），`GET /_bifrost/api/proxy/system` 返回 `enabled=true host=127.0.0.1 port=9900 managed_by_bifrost=true configured_enabled=true`，`reference-candidates` 返回 61 条，`active-summary` 返回 total 2。通过 macOS System Events 展开真实托盘菜单，读取主菜单项为 `Rules: Multiple`，System Proxy 菜单项 `AXMenuItemMarkChar=✓`，Rules 子菜单 `children=8` 且首项 `d` 带 `✓`。 | 通过。真实托盘菜单不再显示 `Rules: None`，`System Proxy` 已按 WebUI live 语义显示勾选；状态来源只信当前实例 Admin API，API 不可达时不猜 OS 状态且不允许错误写请求。 |
...
| 2026-06-22 | TC-TH-30-REG-MEM-01 | 针对用户反馈 macOS tray `MEM` 显示 75%-80%、后续 `M50%` 仍高于实际感知值，以及切到内核 Memory Pressure 后打开/关闭 App 时长期稳定在约 40% 的问题执行口径回归。审查确认旧实现使用 `total - free - speculative` 近似物理非空闲页，会把 inactive / file-backed cache 也算成已用；直接展示 `kern.memorystatus_level` 换算的 pressure 虽更接近 Activity Monitor Memory Pressure 语义，但 pressure 是系统健康度/余量指标，在绿色状态下受缓存、压缩和余量策略影响会明显更钝，不适合做状态栏主读数。最终修复为 macOS 菜单栏 `MEM` 使用偏用户感知的内存负载：`internal - purgeable + wired`，排除 file-backed cache / 可回收缓存，避免缓存误报；下拉系统行保留 `Pressure` 诊断值。菜单栏 MEM 取消 5% 档位量化，改为整数百分比，避免小幅变化被隐藏。新增单测覆盖 file cache 不参与 MEM 百分比、purgeable 扣减、pressure clamp，以及状态栏使用 memory load 而下拉展示 pressure。 | 通过。执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli mac_memory --lib --all-features -- --nocapture` 通过 4/4；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli system_stats --lib --all-features -- --nocapture` 通过 32/32；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli native_ --lib --all-features -- --nocapture` 通过 15/15；执行 `BIFROST_BIN="$PWD/target/debug/bifrost" BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 bash e2e-tests/tests/test_tray_system_stats_config.sh` 通过 63/63。本机对照：`kern.memorystatus_level` 在 `61-67` 小范围波动，换算 pressure 约 `30%-39%`，解释了状态栏若直接使用 pressure 会稳定在约 40%；工作目录 release 构建通过后用 `/Users/eden_studio/work/github/bifrost/target/release/bifrost` 启动真实 9900 服务，主进程 PID `81372`、tray PID `81421`，AX 连续读数包含 `Bifrost: C15% | M30% | D40% | ↑138 K/s ↓11.7 M/s`、`Bifrost: C20% | M30% | D40% | ↑231 K/s ↓11.7 M/s`。锁屏后继续用脚本读取 8 次 AX description，得到 `M31%` 且网络速率持续刷新，例如 `09:08:26 Bifrost: C10% | M31% | D40% | ↑117 K/s ↓8.3 M/s` 到 `09:08:34 Bifrost: C5% | M31% | D40% | ↑44.1 K/s ↓135 K/s`，证明当前运行态使用 memory load 而非 pressure，且锁屏下仍可脚本验证刷新。尝试 `screencapture` 保存 `/tmp/bifrost-tray-mem-load/full-worktree-090502.png`，但输出为黑帧，因此不作为视觉通过证据。 |
| 2026-06-21 | TC-TH-30 | PR `codex/tray-stats-render-perf` 首次远端 CI run `27903460098` fail-fast 于 `Coverage (Unit + Integration) & 90% Gate` job `82567825771`，失败点为 `bifrost-e2e::tests::curl_mock::tests::test_curl_mock_combined` 启动代理时监听 `127.0.0.1:24870` 返回 `Address already in use`。审查 `crates/bifrost-e2e/src/tests/curl_mock.rs` 后确认该文件 12 个 helper 直接使用 `portpicker::pick_unused_port()` 后立即 `ProxyInstance::start`，在 CI 并发覆盖率测试中存在端口 pick 与 bind 之间的竞态；仓库其他 E2E 模块已使用重试换端口模式。修复为新增本地 `start_proxy_with_rules` helper，最多 10 次重试端口绑定竞态，并把 `curl_mock` 内直接启动代理的路径切到该 helper；不改变规则、curl 请求和断言。执行 `cargo fmt --all` 通过；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-e2e tests::curl_mock::tests::test_curl_mock_combined --lib --all-features -- --nocapture` 通过 1/1；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-e2e curl_mock --lib --all-features -- --nocapture` 通过 3/3。 | 通过。远端失败为 CI 端口竞态，不是 tray 产品逻辑回归；已补同文件代理启动重试，覆盖失败单测和同模块公开单测过滤集。 |
| 2026-06-21 | TC-TH-30 | 针对 macOS native tray render 热路径继续做性能优化与真实回归。先从最新 `main` 拉取并切出 `codex/tray-stats-render-perf`；实现上让 `NativeStatsStatusItem` 记录上一份 title，相同展示文本直接跳过渲染；同尺寸 native bitmap 复用 scratch RGBA buffer，避免每秒重新分配；复用同一个 `NSImage` / `NSBitmapImageRep` 时只更新像素并标记 button 重绘，不重复执行 `setLength` / `setImage` / `setImagePosition`；同时把 CPU 与 Memory 采样节流从共享 `last_cpu_memory_at` 拆成独立 `last_cpu_at` / `last_memory_at`，关闭单项后不会因为另一项启用而刷新对应指标。执行 `cargo fmt --all` 通过；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli system_stats --lib --all-features -- --nocapture` 通过 27/27，覆盖 network-only 不刷新 CPU/Memory、CPU-only 不刷新 Memory、Memory-only 不刷新 CPU、Disk 仍按 30 秒节流；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli native_ --lib --all-features -- --nocapture` 通过 14/14，新增覆盖同尺寸 native bitmap 复用同一个 Vec buffer；执行 `SKIP_FRONTEND_BUILD=1 cargo check -p bifrost-cli --all-features` 通过；执行 `SKIP_FRONTEND_BUILD=1 cargo clippy -p bifrost-cli --all-targets --all-features -- -D warnings` 通过；执行 `BIFROST_BIN="$PWD/target/debug/bifrost" BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 bash e2e-tests/tests/test_tray_system_stats_config.sh` 通过 63/63。随后执行 `SKIP_FRONTEND_BUILD=1 cargo build --release --bin bifrost` 通过，使用临时数据目录 `/tmp/bifrost-tray-render-perf.wmDCJB/data`、端口 `58629` 启动当前分支真实 release 实例，主进程 PID `78328`、tray PID `78342`，启动参数包含 `--no-system-proxy --skip-cert-check` 且设置 `BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1`；`GET /_bifrost/api/config/tray` 返回 CPU/Memory/Disk/Upload/Download 全部启用；warm-up 15 秒后采集 60 个 1 秒 `ps` 样本，CSV 为 `/tmp/bifrost-tray-render-perf.wmDCJB/tray-ps-sample-release.csv`。为了判断是否真的节省资源，另建独立 worktree `/tmp/bifrost-tray-perf-main-baseline` 指向 `origin/main`，构建 baseline release 到 `/tmp/bifrost-tray-perf-main-baseline-target/release/bifrost`，使用临时目录 `/tmp/bifrost-tray-main-baseline-perf.DzGVtu/data`、端口 `58860`、tray PID `86839` 执行同样 60 秒采样，CSV 为 `/tmp/bifrost-tray-main-baseline-perf.DzGVtu/tray-ps-sample-release.csv`。 | 通过。两轮采样均为真实 macOS release tray helper、系统状态全开、1 秒采样、未启用系统代理。`origin/main` baseline 汇总为 `samples=60 avg_cpu=0.3500 min_cpu=0.0000 p50=0.2000 p90=1.0000 p95=1.3000 max_cpu=1.5000 avg_rss_kb=68244 min_rss_kb=66624 max_rss_kb=68912 rss_delta_kb=2288`；当前优化分支汇总为 `samples=60 avg_cpu=0.3150 min_cpu=0.0000 p50=0.2000 p90=0.9000 p95=1.0000 max_cpu=1.1000 avg_rss_kb=68343 min_rss_kb=66768 max_rss_kb=69024 rss_delta_kb=2256`。结论：保持 1 秒网络刷新和相同视觉行为时，平均 CPU 从 0.350% 降到 0.315%（约 10%），p95 从 1.3% 降到 1.0%，max 从 1.5% 降到 1.1%；RSS 无显著增长。这是稳定性和峰值优化，不是数量级下降。baseline worktree 已移除。 |
