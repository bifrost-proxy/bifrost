...
| 2026-06-22 | TC-TH-30-REG-MEM-01 | 针对用户反馈 macOS tray `MEM` 显示 75%-80% 以及后续 `M50%` 仍高于实际感知值执行口径回归。审查确认旧实现使用 `total - free - speculative` 近似物理非空闲页，会把 inactive / file-backed cache 也算成已用；第一轮 `internal - purgeable + wired` 虽排除了 file-backed cache 和 compressor occupied，但仍会把冷的 inactive anonymous 页纳入当前压力，导致本机继续显示约 50%。最终修复为 macOS `MEM` 使用偏用户感知的即时活跃压力口径：`active + wired - purgeable`，排除 inactive / file-backed cache / 可回收缓存，也不把 long-lived compressed store 或冷匿名页当成当前活跃消耗；如果 active 字段不可用，则先 fallback 到 `internal - purgeable + wired`，极端字段为空时才 fallback 到非空闲页估算。新增单测覆盖 file cache 不参与 MEM 百分比、active 字段为空时 fallback 到 internal，以及压力字段为空时 fallback 不返回 0。 | 通过。执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli mac_memory_used_pages --lib --all-features -- --nocapture` 通过 3/3；执行 `SKIP_FRONTEND_BUILD=1 cargo test -p bifrost-cli system_stats --lib --all-features -- --nocapture` 通过 30/30；执行 `BIFROST_BIN="$PWD/target/debug/bifrost" BIFROST_SYNC_DISABLE_AUTO_LOGIN_PROMPT=1 bash e2e-tests/tests/test_tray_system_stats_config.sh` 通过 63/63。本机页数对照：旧非空闲口径约 `99.79%`、第一轮 `internal - purgeable + wired` 约 `55.50%`、最终 `active + wired - purgeable` 约 `38.91%`；安装当前 release 到 `/Users/eden_studio/.cargo/bin/bifrost` 与 `/Users/eden_studio/.local/bin/bifrost` 后重启真实 9900 服务，主进程 PID `1316`、tray PID `1362`，截图 `/tmp/bifrost-tray-mem-pressure-active/topbar-full.png` 可见菜单栏状态项显示 `M35%`。 |
