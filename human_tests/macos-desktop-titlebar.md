...
| 2026-07-05 | TC-MDT-01 / 04 / 05 | 执行 `BIFROST_DESKTOP_NO_SYSTEM_PROXY=1 BIFROST_DESKTOP_SKIP_CERT_PREFLIGHT=1 pnpm run desktop:dev` 重新构建并启动真实 macOS Tauri 窗口；截图 `/tmp/bifrost-titlebar-shots/network-scoped-transparent.png` 验证 Network 仅顶部工具栏和 Filters 左栏透出窗口背景，表格与详情保持原深色信息底；用户截图确认 Replay 左侧 Collections 与窗口背景融合、右侧请求/响应工作区恢复原背景；Notify / Groups / DevTools 的页面顶层 padding 按全平台规则收敛，不再额外叠加顶部空白。 | 通过：Network 和 Replay 的透明区域已限定到用户指定范围，主要工作区底色未被破坏；顶部视觉节奏按全平台一致性处理 |
