...
> 规则侧如何引用脚本（`reqScript://` / `resScript://` / `resStreamScript://` / `decode://` / `bp://`）请参考[脚本规则参考](./rules/scripts.md)。通过 CLI 或 AI 生成、分享规则时，默认推荐把脚本写成规则文件内的 inline block；Scripts 页面主要用于交互式测试、试跑和人工维护命名脚本。
...
完整的 inline block 写法、event 字段、返回值和响应头约束见[脚本规则参考](./rules/scripts.md)。
